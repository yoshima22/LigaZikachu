...
import {
  personalityOffenseMult,
  personalityDefenseMult,
  debuffResistanceFactor,
  personalityAgilityMult,
  rollPlayfulTeamBuff,
  rollTravessoDebuff,
} from "@/lib/personality-combat";
import {
  getPokemonElement,
  getPokemonName,
  getPokemonTypes,
...
export const PVE_REWARD_MULT = 0.4; // vitória PvE vale ~40% do original (era 30%)
...
export async function applyPassiveIncome(
  teamId: string,
): Promise<{ coins: number; exp: number } | null> {
  ...
  if (!team || team.status !== "ACTIVE" || team.members.length === 0)
    return null;
...
export async function applyPassiveIncomeForPlayer(
  playerId: string,
): Promise<void> {
  ...
  await Promise.all(
    activeTeams.map((t) => applyPassiveIncome(t.id).catch(() => null)),
  );
}
...
export function getTeamTimeMultiplier(enteredAt: Date): number {
  ...
  return Math.min(
    ARENA_Z_CONFIG.multCap,
    1 + hoursActive * ARENA_Z_CONFIG.multPerHour,
  );
}
...
export function estimateVaultClaim(
  ...
  const estimatedPvpCoins = Math.floor(
    cappedHours * memberCount * PASSIVE_COINS_PER_MASCOT_PER_H,
  );
  const estimatedPvpExp = Math.floor(
    cappedHours * memberCount * PASSIVE_EXP_PER_MASCOT_PER_H,
  );
...
export const DIFFICULTY_CONFIG = {
  easy: {
    levelOffset: -3,
    rewardMult: 0.8,
    injuryChanceMult: 0.6,
    label: "Facil",
    color: "green",
    desc: "Bot abaixo da forca real do time. Menos loot e risco menor.",
  },
  normal: {
    levelOffset: +3,
    rewardMult: 1.2,
    injuryChanceMult: 1.2,
    label: "Normal",
    color: "yellow",
    desc: "Bot acima da forca real do time, evitando abuso de media baixa.",
  },
  hard: {
    levelOffset: +8,
    rewardMult: 2.0,
    injuryChanceMult: 2.8,
    label: "Dificil",
    color: "red",
    desc: "Bot bem acima da forca real. Loot maior, risco alto de ferimento.",
  },
} as const;
...
function arenaTypes(m: ArenaMascot) {
  return m.types?.length ? m.types : getPokemonTypes(m.pokemonId);
}
function arenaElement(m: ArenaMascot) {
  return arenaTypes(m)[0] ?? getPokemonElement(m.pokemonId);
}
...
export type ArenaCombatRuntime = {
  ...
  survivorUsed: string[];
  duelistLock: Record<string, string>;
  hitTaken: string[];
  dramaticSaveUsed: string[];
...
function toArenaMascot(
  ...
  return {
    ...
    types: m.primaryTypeOverride
      ? ([m.primaryTypeOverride, m.secondaryTypeOverride].filter(
          Boolean,
        ) as string[])
    ...
    hp: Math.max(
      10,
      Math.round((55 + m.level * 6 + m.statVitality * 4) * mult),
    ),
    ...
    combatRole: normalizeCombatRole(
      m.combatRole ??
...
function makeBotMascot(
  ...
  const pokemonIds = [
    1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 15, 18, 20, 22, 24, 25, 26, 28, 31, 34, 36,
    38, 40, 45, 49, 51, 53, 55, 57, 59, 62, 65, 68, 71, 73, 76, 78, 80, 82, 85,
    87, 89, 91, 94, 95, 97, 99, 101, 103, 105, 106, 107, 110, 112, 115, 117,
    119, 121, 123, 124, 125, 126, 127, 128, 130, 131, 134, 135, 136, 139, 141,
    143, 149, 154, 157, 160, 162, 164, 166, 168, 169, 171, 178, 181, 184, 185,
    186, 189, 195, 196, 197, 199, 203, 205, 208, 210, 212, 214, 217, 219, 221,
    224, 226, 229, 230, 232, 237, 241, 242, 248,
  ];
...
function aliveEncourageBonus(team: ArenaMascot[], hp: Map<string, number>) {
  const encouragers = team.filter(
    (m) => m.combatRole === "ENCOURAGER" && (hp.get(m.id) ?? 0) > 0,
  );
...
function chooseRoleTarget(
  ...
  if (actor.combatRole === "ATTACKER")
    return (
      [...opponents].sort((a, b) => b.force - a.force)[0] ?? pick(opponents)
    );
  if (actor.combatRole === "OPPORTUNIST")
    return (
      [...opponents].sort((a, b) => a.instinct - b.instinct)[0] ??
      pick(opponents)
    );
  if (actor.combatRole === "DUELIST")
    return (
      [...opponents].sort((a, b) => b.force - a.force)[0] ?? pick(opponents)
    );
...
function roleDamageMultiplier(actor: ArenaMascot, target: ArenaMascot) {
  ...
  // Original 5 roles
  if (actor.combatRole === "ATTACKER")
    mult *= 1 + getAttackerDamageBonus(actor.force);
  if (actor.combatRole === "ATTACKER" && target.combatRole === "DEFENDER")
    mult *= 1.15;
  if (actor.combatRole === "FLANK")
    mult *= 1 + getFlankDamageBonus(actor.agility);
  if (
    actor.combatRole === "FLANK" &&
    ["ENCOURAGER", "OPPORTUNIST", "HEALER"].includes(target.combatRole)
  )
    mult *= 1.12;
  if (actor.combatRole === "OPPORTUNIST" && actor.instinct > target.instinct)
    mult *= 1.1;
  // New 8 roles
  if (actor.combatRole === "DUELIST")
    mult *= 1 + getDuelistDamageBonus(actor.force, actor.instinct);
  ...
  if (actor.combatRole === "GUARDIAN") mult *= 0.9;
  if (actor.combatRole === "HEALER") mult *= 0.8;
  // Defensive roles
  if (target.combatRole === "DEFENDER")
    mult *= 1 - getDefenderReduction(target.vitality);
  if (target.combatRole === "FLANK")
    mult *= 1 - Math.min(0.25, target.agility / 360);
  if (target.combatRole === "GUARDIAN")
    mult *= 1 - getGuardianReduction(target.vitality);
...
function tryApplyOpportunistDebuff(
  actor: ArenaMascot,
  target: ArenaMascot,
  debuffs: Map<
    string,
...
function trySaboteurDisrupt(
  ...
  if (encouragers.length === 0)
    return `Sabotador ${actor.name} interferiu no ritmo inimigo.`;
...
function tryGuardianIntercept(
  ...
  const guardians = allies.filter(
    (m) =>
      m.combatRole === "GUARDIAN" &&
      m.id !== target.id &&
      (hp.get(m.id) ?? 0) > 0,
  );
  ...
  return {
    newDamage: remaining,
    guardianDamage: absorbed,
    guardianId: guardian.id,
    effect: `Guardião ${guardian.name} protegeu ${target.name}, absorvendo ${absorbed} de dano.`,
  };
}

function trySurvivorLastStand(
  target: ArenaMascot,
  damage: number,
  hp: Map<string, number>,
  survivorUsed: Set<string>,
): { newDamage: number; effect: string } | null {
  ...
  return {
    newDamage: currentHp - 1,
    effect: `Sobrevivente ${target.name} resistiu ao golpe fatal e sobreviveu com 1 HP!`,
  };
}

function tryProvokerRedirect(
  actor: ArenaMascot,
  target: ArenaMascot,
  opponents: ArenaMascot[],
  hp: Map<string, number>,
): ArenaMascot | null {
  const provokers = opponents.filter(
    (m) =>
      m.combatRole === "PROVOKER" &&
      m.id !== target.id &&
      (hp.get(m.id) ?? 0) > 0,
  );
...
function aliveScoutBonus(team: ArenaMascot[], hp: Map<string, number>) {
  const scouts = team.filter(
    (m) => m.combatRole === "SCOUT" && (hp.get(m.id) ?? 0) > 0,
  );
...
function aliveSaboteurSuppression(
  opponents: ArenaMascot[],
  hp: Map<string, number>,
  target?: ArenaMascot,
) {
  const saboteurs = opponents.filter(
    (m) => m.combatRole === "SABOTEUR" && (hp.get(m.id) ?? 0) > 0,
  );
  if (saboteurs.length === 0) return 0;
  const best = saboteurs.sort(
    (a, b) => b.instinct + b.agility - (a.instinct + a.agility),
  )[0];
  return (
    getSaboteurSuppression(best.instinct, best.agility) *
    (target ? debuffResistanceFactor(best, target) : 1)
  );
}
...
export function runArenaCombat(
  ...
  const hp = new Map<string, number>(Object.entries(previous?.hp ?? {}));
  for (const m of [...attackers, ...defenders])
    if (!hp.has(m.id)) hp.set(m.id, m.hp);

  const log: ArenaTurnLog[] = [];
  const debuffs = new Map<
    string,
    Partial<Record<"force" | "agility" | "instinct" | "vitality", number>>
  >(Object.entries(previous?.debuffs ?? {}));
  const healCount = new Map<string, number>(
    Object.entries(previous?.healCount ?? {}),
  );
  const survivorUsed = new Set<string>(previous?.survivorUsed ?? []);
  const duelistLock = new Map<string, string>(
    Object.entries(previous?.duelistLock ?? {}),
  );
  // Estado dos efeitos de personalidade:
  const hitTaken = new Set<string>(previous?.hitTaken ?? []);
  const dramaticSaveUsed = new Set<string>(previous?.dramaticSaveUsed ?? []);
  ...
  while (
    ...
    for (const entry of all) {
      ...
      const effAgility =
        actor.agility *
        personalityAgilityMult(actor, round, playfulTeamBuff[entry.side]);
      const actionProfile = getCombatActionsPerRound(
        effAgility,
        alive(enemyTeam, hp).map((m) =>
          getEffectiveStat(m, debuffs, "agility"),
        ),
      );

      for (
        let actionIndex = 0;
        actionIndex < actionProfile.actions;
        actionIndex++
      ) {
        ...
        if (actor.combatRole === "DUELIST" && duelistLock.has(actor.id)) {
          const locked = opponents.find(
            (m) => m.id === duelistLock.get(actor.id),
          );
        ...
        if (actor.combatRole === "DUELIST")
          duelistLock.set(actor.id, target.id);
        ...
        const bestEncourager = allies
          .filter(
            (m) => m.combatRole === "ENCOURAGER" && (hp.get(m.id) ?? 0) > 0,
          )
          .sort((a, b) => b.charisma - a.charisma)[0];
        const saboteurSuppression = aliveSaboteurSuppression(
          opponents,
          hp,
          bestEncourager,
        );
        ...
        const survivorDmgBonus =
          actor.combatRole === "SURVIVOR" && actorHpPct < 0.3 ? 1.15 : 1;

        // SURVIVOR: extra reduction when low HP (target)
        const targetHpPct = (hp.get(target.id) ?? 0) / target.hp;
        const survivorDefBonus =
          target.combatRole === "SURVIVOR" && targetHpPct < 0.3 ? 0.75 : 1;
        ...
        if (persOff > 1) {
          ...
          if (actor.personality === "DRAMATIC")
            persOffNote = `Dramático ${actor.name} entrou em fúria com HP baixo (+${pct}% de dano).`;
          else if (actor.personality === "PROUD")
            persOffNote = `Orgulho de ${actor.name} em alta: personalidade rende +${pct}% de dano.`;
          else if (actor.personality === "COMPETITIVE")
            persOffNote = `Competitivo ${actor.name} encara um alvo mais forte: personalidade rende +${pct}% de dano.`;
          else if (actor.personality === "TIMID")
            persOffNote = `Tímido ${actor.name} reage após ser atingido: personalidade rende +${pct}% de dano.`;
          else if (actor.personality === "CURIOUS")
            persOffNote = `Curioso ${actor.name} estuda o oponente: personalidade rende +${pct}% de dano.`;
        }
        ...
        let loyalMult = 1;
        let loyalNote: string | null = null;
        if (loyalMate) {
          const mateHp = hp.get(loyalMate.id) ?? 0;
          if (mateHp > 0 && mateHp / loyalMate.hp < 0.35) {
            loyalMult = 1.05;
            loyalNote = `🤝 Leal ${actor.name} defende ${loyalMate.name} em perigo (+5%).`;
          } else loyalNote = `🤝 Leal ${actor.name} protege ${loyalMate.name}.`;
        }
        const raw =
          (force * 1.8 + actor.level * 2 + instinct * 0.7 + rand(0, 12)) *
          (1 + encourage + scoutBonus) *
          roleDamageMultiplier(actor, target) *
          duelistBonus *
          survivorDmgBonus *
          persOff *
          loyalMult;
        ...
        const guardianIntercept = tryGuardianIntercept(
          target,
          entry.side === "A" ? defenders : attackers,
          hp,
          damage,
        );
        ...
        const guardianKnockedOut = Boolean(
          guardianIntercept &&
          (hp.get(guardianIntercept.guardianId) ?? 0) === 0,
        );
        if (
          (newHp === 0 || guardianKnockedOut) &&
          actor.personality === "DRAMATIC"
        ) {
        ...
        const effects =
          [
            actionIndex > 0
              ? `Agilidade: ação extra (${actionIndex + 1}/${actionProfile.actions}).`
              : null,
            encourage > 0
              ? `Encorajador ativo: +${Math.round(encourage * 100)}% impulso.`
              : null,
            scoutBonus > 0
              ? `Batedor ativo: +${Math.round(scoutBonus * 100)}% precisão.`
              : null,
            loyalNote,
            persOffNote,
            debuffEffect,
            ...
            survivorLS?.effect,
            provoked
              ? `Provocador ${provoked.name} desviou o ataque para si!`
              : null,
            actor.combatRole === "DUELIST"
  ...
  const aHp = alive(attackers, hp).reduce(
    (sum, m) => sum + (hp.get(m.id) ?? 0),
    0,
  );
  const dHp = alive(defenders, hp).reduce(
    (sum, m) => sum + (hp.get(m.id) ?? 0),
    0,
  );
  const result: ArenaBattleResult =
    aHp === dHp ? "DRAW" : aHp > dHp ? "ATTACKER_WIN" : "DEFENDER_WIN";
  const defeated = [...attackers, ...defenders].filter(
    (m) => (hp.get(m.id) ?? 0) <= 0,
  );
  const runtime: ArenaCombatRuntime = {
    turn,
    round,
    hp: Object.fromEntries(hp),
    debuffs: Object.fromEntries(debuffs),
    healCount: Object.fromEntries(healCount),
    survivorUsed: [...survivorUsed],
    duelistLock: Object.fromEntries(duelistLock),
    hitTaken: [...hitTaken],
    dramaticSaveUsed: [...dramaticSaveUsed],
  ...
  return {
    ...
    log,
    rounds: turn - 1,
    defeatedMascotIds: defeated.filter((m) => m.ownerId).map((m) => m.id),
    runtime,
    finished,
  };
}
...
function botReward(
  ...
  if (difficulty === "hard" && Math.random() < 0.2) {
    // 20% de chance no Difícil
    const pool =
      Math.random() < 0.2 ? [...LEAGUE_SHOP_ITEM_TYPES] : BUFF_ITEM_POOL;
...
function getBotRewardRange(
  levelMax: number,
  diffRewardMult = 1.2 /* Normal */,
) {
  const tier = Math.ceil(levelMax / 5);
...
function getEffectiveTeamLevel(attackers: ArenaMascot[]) {
  ...
  const avgLevel =
    levels.reduce((sum, level) => sum + level, 0) / Math.max(1, levels.length);
  const topCount = Math.max(1, Math.ceil(levels.length / 2));
  const topAvg =
    levels.slice(0, topCount).reduce((sum, level) => sum + level, 0) / topCount;
  const maxLevel = levels[0] ?? 1;
  return Math.max(
    1,
    Math.round(Math.max(avgLevel, topAvg * 0.9, maxLevel * 0.72)),
  );
}
...
function buildBotOpponent(
  ...
  const nameSeed =
    seed !== undefined
      ? seed % BOT_NAMES.length
      : Math.floor(rng() * BOT_NAMES.length);
  const botName = BOT_NAMES[nameSeed];
  const defenders = Array.from({ length: botSize }, (_, index) =>
    makeBotMascot(index + 1, band.min, band.max, rng),
  );
  return {
    avgLevel,
    band,
    ...
    botName,
    defenders,
    rewardRange: getBotRewardRange(band.max, diff.rewardMult),
    difficulty,
  };
}
...
export async function getArenaBotPreview(
  ...
  if (
    ...
    return null;
  const attackers = team.members.map((m) =>
    toArenaMascot({ ...m.mascot, combatRole: m.combatRole }),
  );
  ...
  const seed =
    parseInt(teamId.replace(/[^0-9]/g, "").slice(-4) || "0") + hourSlot;
  const bot = buildBotOpponent(attackers, difficulty, seed);
  const diff = DIFFICULTY_CONFIG[difficulty];
  // Último combate da equipe (para mostrar cooldown)
  const hasPrefetchedCooldown =
    options && Object.prototype.hasOwnProperty.call(options, "lastBattleAt");
  const lastBattle = hasPrefetchedCooldown
    ? options?.lastBattleAt
      ? { createdAt: options.lastBattleAt }
      : null
  ...
  const cooldownUntil = lastBattle
    ? new Date(
        lastBattle.createdAt.getTime() +
          ARENA_Z_CONFIG.botCooldownMinutes * 60_000,
      )
    : null;
  return {
    ...
    cooldownMs: cooldownUntil
      ? Math.max(0, cooldownUntil.getTime() - Date.now())
      : 0,
...
export async function getRoomsData(viewerPlayerId?: string) {
  ...
  if (viewerPlayerId) {
    const foughtBattles = await prisma.arenaBattle.findMany({
      where: {
        ...
        type: { in: ["PVP"] },
        defenseTeamId: { not: null },
      },
      select: { defenseTeamId: true },
      distinct: ["defenseTeamId"],
      take: 200,
    });
    foughtBattles.forEach((b) => {
      if (b.defenseTeamId) revealedTeamIds.add(b.defenseTeamId);
    });
  }
  ...
  function mapTeam(t: (typeof teams)[number], viewerId?: string) {
    ...
    return {
      ...
      members: t.members.map((m) => ({
        ...
        name: revealed
          ? (m.mascot.nickname ?? getPokemonName(m.mascot.pokemonId))
          : null,
      })),
    };
  }
  ...
  const rooms = ARENA_ROOMS.map((roomLevel) => {
    const roomTeams = teams.filter(
      (t) => !t.isTraining && t.roomLevel === roomLevel,
    );
    return {
      roomLevel,
      teamCount: roomTeams.length,
      teams: roomTeams.map((t) => mapTeam(t, viewerPlayerId)),
    };
  });
  ...
  const trainingRoom = {
    roomLevel: 0,
    isTraining: true,
    teamCount: trainingTeams.length,
    teams: trainingTeams.map((t) => mapTeam(t, viewerPlayerId)),
  };
...
async function dropArenaGroundSpoils(
  ...
  if ((!loot || !hasLoot(loot)) && (!itemPayload || itemPayload.length === 0))
    return;
  await tx.arenaGroundSpoil.create({
    data: {
      ...
      sweet: loot?.sweet ?? 0,
      itemPayload:
        itemPayload && itemPayload.length > 0
          ? (itemPayload as unknown as Prisma.InputJsonValue)
          : undefined,
      status: "AVAILABLE",
    },
  });
...
async function maybeFindArenaGroundSpoils(
  ...
  await tx.arenaGroundSpoil.update({
    ...
    data: {
      status: "CLAIMED",
      foundByPlayerId: winnerPlayerId,
      claimedAt: new Date(),
    },
  });
    ...
    if (item.kind === "MASCOT_BUFF" && item.type) {
      await tx.playerGift.create({
        data: {
          ...
          description:
            "Item encontrado no chao da Arena Z apos uma vitoria PvP. Resgate para enviar ao inventario.",
          payload: {
...
async function creditArenaLoot(
  ...
  if (payableCoins > 0) {
    ...
    const earnedToday =
      player?.arenaPveCoinsDate === todayBRT
        ? (player.arenaPveCoinsEarned ?? 0)
        : 0;
...
async function getArenaExpMascotIds(
  teamId: string,
  playerId: string,
  enteredAt: Date,
): Promise<string[]> {
  ...
  for (const battle of battles) {
    ...
    for (const rawTurn of turns) {
      ...
      const turn = rawTurn as {
        actorId?: unknown;
        actorOwnerId?: unknown;
        targetId?: unknown;
        targetOwnerId?: unknown;
      };
      if (turn.actorOwnerId === playerId && typeof turn.actorId === "string")
        ids.add(turn.actorId);
      if (turn.targetOwnerId === playerId && typeof turn.targetId === "string")
        ids.add(turn.targetId);
    }
  }
...
async function distributeArenaExp(
  mascotIds: string[],
  totalExp: number,
): Promise<void> {
  if (totalExp <= 0) return;
  if (mascotIds.length === 0)
    throw new Error("Nenhum mascote elegivel para receber EXP da Arena.");
  const expPerMascot = Math.max(1, Math.floor(totalExp / mascotIds.length));
  const results = await Promise.allSettled(
    mascotIds.map((mascotId) =>
      addExp(mascotId, expPerMascot, { ignoreBenchPenalty: true }),
    ),
  );
  const failed = results.filter((result) => result.status === "rejected");
  if (failed.length > 0) {
    throw new Error(
      `Falha ao distribuir EXP da Arena para ${failed.length}/${mascotIds.length} mascote(s).`,
    );
  }
}
...
function assertTeamReady(team: {
  ...
  for (const member of team.members) {
    ...
    if (member.mascot.arenaState === "INJURED")
      throw new Error(`${name} esta ferido e precisa de Atendimento SUS.`);
    if (member.mascot.restingUntil && member.mascot.restingUntil > now)
      throw new Error(
        `${name} esta em repouso ate ${member.mascot.restingUntil.toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo" })}.`,
      );
  }
}

export async function validateArenaMascots(
  playerId: string,
  mascotIds: string[],
) {
  const unique = [...new Set(mascotIds)].filter(Boolean);
  if (unique.length < 1 || unique.length > 6)
    throw new Error("Selecione de 1 a 6 mascotes.");
  ...
  for (const m of mascots) {
    ...
    if (m.bazarListed)
      throw new Error(
        `${m.nickname ?? getPokemonName(m.pokemonId)} esta anunciado no Bazar.`,
      );
    if (m.arenaState === "INJURED")
      throw new Error(
        `${m.nickname ?? getPokemonName(m.pokemonId)} esta ferido.`,
      );
    if (m.arenaState === "RESTING" && m.restingUntil && m.restingUntil > now)
      throw new Error(
        `${m.nickname ?? getPokemonName(m.pokemonId)} esta em repouso.`,
      );
    if (m.arenaState === "TRACE_HIDING" || m.arenaState === "TRACE_HUNTING")
      throw new Error(
        `${m.nickname ?? getPokemonName(m.pokemonId)} esta em Cacada de Rastros.`,
      );
    if (m.arenaState === "ARENA")
      throw new Error(
        `${m.nickname ?? getPokemonName(m.pokemonId)} ja esta em uma equipe da Arena.`,
      );
    ...
    if (m.arenaState === "FREE" && m.restingUntil && m.restingUntil > now) {
      const rem = Math.ceil(
        (m.restingUntil.getTime() - now.getTime()) / 60_000,
      );
      throw new Error(
        `${m.nickname ?? getPokemonName(m.pokemonId)} ainda esta em cooldown de re-entrada na Arena (${rem} min).`,
      );
    }
  }
  return mascots;
...
async function checkRetireCooldown(mascotIds: string[]): Promise<void> {
  ...
  if (!blocked) return;
  const name =
    blocked.mascot.nickname ?? getPokemonName(blocked.mascot.pokemonId);
  const retiredMs = blocked.team.retiredAt
    ? new Date(blocked.team.retiredAt).getTime()
    : Date.now();
  const remaining = Math.ceil(
    (RETIRE_COOLDOWN_MS - (Date.now() - retiredMs)) / 60_000,
  );
  throw new Error(
    `${name} precisa esperar ${remaining} min antes de entrar em nova equipe (saiu com recompensas recentemente).`,
  );
}

function applyBondModifiersToArenaMascots(
  team: ArenaMascot[],
...
async function assertPvpExitUnlocked(teamId: string): Promise<void> {
  const since = new Date(
    Date.now() - ARENA_Z_CONFIG.pvpExitLockMinutes * 60_000,
  );
  ...
  const unlockAt =
    recentAttack.createdAt.getTime() +
    ARENA_Z_CONFIG.pvpExitLockMinutes * 60_000;
  const remaining = Math.max(1, Math.ceil((unlockAt - Date.now()) / 1000));
  const min = Math.floor(remaining / 60);
  const sec = remaining % 60;
  throw new Error(
    `Esta equipe atacou outro jogador recentemente. Aguarde ${min}:${sec.toString().padStart(2, "0")} para abandonar ou coletar recompensas.`,
  );
}
...
export async function createArenaTeam(
  ...
  name: string,
  mascotIds: string[],
  roomLevel: ArenaRoom = 100,
  combatRoles?: Record<string, string>,
  isTraining = false,
) {
  if (mascotIds.length > 6) throw new Error("Máximo de 6 mascotes por equipe.");
  if (!isTraining && !ARENA_ROOMS.includes(roomLevel as ArenaRoom))
  ...
  if (!isTraining) {
    for (const m of mascots) {
      if (m.level > roomLevel) {
        throw new Error(
          `${m.nickname ?? getPokemonName(m.pokemonId)} (nível ${m.level}) está acima do máximo da sala (nível ${roomLevel}).`,
        );
      }
    }
  }
  ...
  const activeCount = await prisma.arenaTeam.count({
    where: { playerId, status: "ACTIVE" },
  });
  if (activeCount >= ARENA_MAX_TEAMS) {
    throw new Error(
      `Você já tem ${ARENA_MAX_TEAMS} equipes ativas na Arena. Retire uma antes de criar uma nova.`,
    );
  }
  ...
  const teamName =
    name.trim() ||
    `Time do ${firstName} de ${player?.displayName ?? "Jogador"}`;
  ...
  const alreadyInTeam = await prisma.arenaTeamMember.findMany({
    where: {
      mascotId: { in: mascots.map((m) => m.id) },
      team: { status: "ACTIVE" },
    },
    include: { team: { select: { name: true } } },
  });
  if (alreadyInTeam.length > 0) {
    const names = alreadyInTeam.map(
      (m) =>
        `Mascote (${m.mascotId.slice(-4)}) já está na equipe "${m.team.name}"`,
    );
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (activeLinks.length > 0) {
        throw new Error(
          `Mascote ja esta na equipe "${activeLinks[0].team.name}". Atualize a pagina e tente novamente.`,
        );
      }

      // Reconfirma limite 3 dentro da transação
      const activeNow = await tx.arenaTeam.count({
        where: { playerId, status: "ACTIVE" },
      });
      if (activeNow >= ARENA_MAX_TEAMS)
      ...
      if (lockedMascots.count !== mascots.length) {
        throw new Error(
          "Um mascote deixou de estar disponivel. Atualize a pagina e tente novamente.",
        );
      }
        ...
        data: {
          ...
          members: {
            create: mascots.map((m, index) => ({
              mascotId: m.id,
              slot: index + 1,
              combatRole: combatRoles?.[m.id]
                ? normalizeCombatRole(combatRoles[m.id])
                : defaultCombatRoleFor(m),
            })),
          },
        },
      });
...
export async function addMascotToArenaTeam(
  playerId: string,
  teamId: string,
  mascotId: string,
) {
  ...
  if (team.status !== "ACTIVE")
    throw new Error("Apenas equipes ativas podem receber mascotes.");
  ...
  if (!team.isTraining && mascot.level > (team.roomLevel ?? 999)) {
    throw new Error(
      `${mascot.nickname ?? getPokemonName(mascot.pokemonId)} (nível ${mascot.level}) está acima do limite desta sala (nível ${team.roomLevel}). Escolha um mascote dentro do limite.`,
    );
  }
  const usedSlots = new Set(team.members.map((member) => member.slot));
  const slot = [1, 2, 3, 4, 5, 6].find(
  ...
  return prisma.$transaction(
    async (tx) => {
      ...
      if (activeLink)
        throw new Error(
          `Mascote ja esta na equipe "${activeLink.team.name}". Atualize a pagina e tente novamente.`,
        );
      ...
      if (locked.count !== 1)
        throw new Error(
          "Mascote deixou de estar disponivel. Atualize a pagina e tente novamente.",
        );
      ...
      const member = await tx.arenaTeamMember.create({
        data: {
          ...
          mascotId: mascot.id,
          slot,
          combatRole: defaultCombatRoleFor(mascot),
        },
      });
      return member;
    },
...
export async function setArenaTeamMemberCombatRole(
  playerId: string,
  teamId: string,
  mascotId: string,
  combatRole: string,
  ...
  if (!team || team.playerId !== playerId)
    throw new Error("Equipe nao encontrada.");
  if (team.status !== "ACTIVE")
    throw new Error("Apenas equipes ativas podem alterar postura.");
  ...
  if (updated.count === 0)
    throw new Error("Mascote nao encontrado nesta equipe.");
}
 ...
 */
export async function deleteArenaTeam(
  playerId: string,
  teamId: string,
  isAdmin = false,
) {
  const team = await prisma.arenaTeam.findUnique({
    ...
    include: {
      members: true,
      player: { include: { user: { select: { role: true } } } },
    },
  });
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.mascot.updateMany({
      where: {
        id: { in: team.members.map((m) => m.mascotId) },
...
export async function purgeAdminArenaData(): Promise<{
  teams: number;
  battles: number;
}> {
...
export async function retireArenaTeam(playerId: string, teamId: string) {
  ...
  if (!team || team.playerId !== playerId)
    throw new Error("Equipe nao encontrada.");
  if (!["ACTIVE", "DEFEATED"].includes(team.status))
    throw new Error("Equipe ja retirada.");
  ...
  if (unseenPvp) {
    throw Object.assign(
      new Error(
        `Voce foi atacado por ${unseenPvp.attackerName} antes de sair. Visualize o combate e tente novamente.`,
      ),
      { unseenPvp },
    );
  }
  ...
  const refreshed = await prisma.arenaTeam.findUnique({
    where: { id: teamId },
    include: { members: true },
  });
  if (refreshed) {
    team.vaultCoins = refreshed.vaultCoins;
    team.vaultExp = refreshed.vaultExp;
    team.vaultFood = refreshed.vaultFood;
    team.vaultSweet = refreshed.vaultSweet;
  }
  ...
  const hadRecentBattle = lastBattleAt
    ? Date.now() - lastBattleAt.getTime() <
      ARENA_Z_CONFIG.pvpQuickExitPenaltyHours * 3_600_000
    : false;

  // Todas as equipes são BOTH — multiplicador de tempo aplica apenas à renda passiva PvP estimada
  const hoursActive =
    (Date.now() - new Date(team.enteredAt).getTime()) / 3_600_000;
  const cappedHours = Math.min(hoursActive, PASSIVE_MAX_HOURS);
  ...
  const pvpPart = applyMultiplierToVault(
    { coins: pvpCoins, exp: pvpExp, food: 0, sweet: 0 },
    mult,
    hadRecentBattle,
  );
  ...
  const player = await prisma.player.findUnique({
    where: { id: playerId },
    select: {
      arenaPveCoinsDate: true,
      arenaPveCoinsEarned: true,
      displayName: true,
    },
  });
  const todayBRT = new Date().toLocaleDateString("sv-SE", {
  ...
  const expMascotIds = await getArenaExpMascotIds(
    team.id,
    playerId,
    team.enteredAt,
  );
  if (vaultFinal.exp > 0 && expMascotIds.length === 0) {
    throw new Error(
      "Nao foi possivel identificar quais mascotes devem receber a EXP do cofre. A equipe nao foi retirada.",
    );
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const entryCooldownUntil = hadCoins
      ? new Date(Date.now() + RETIRE_COOLDOWN_MS)
      : null;
    for (const member of team.members) {
      await tx.mascot.updateMany({
...
export async function syncDefeatedArenaTeams(playerId: string) {
  ...
  const activeMascotIds = new Set(
    teams.flatMap((team) => team.members.map((member) => member.mascotId)),
  );
  const updated = {
    teams: 0,
    deletedEmptyTeams: 0,
    releasedOrphans: 0,
    removedInvalidMembers: 0,
  };
  ...
  await prisma.$transaction(async (tx) => {
    for (const team of teams) {
      ...
      const survivors = team.members.filter(
        (member) => !invalidMembers.some((invalid) => invalid.id === member.id),
      );
      ...
      const legacyResting = survivors.filter(
        (member) =>
          member.mascot.restingUntil || member.mascot.arenaState === "RESTING",
      );
      if (legacyResting.length > 0) {
        await tx.mascot.updateMany({
          where: {
            id: { in: legacyResting.map((member) => member.mascotId) },
            arenaState: { not: "INJURED" },
          },
    ...
    const orphans = await tx.mascot.updateMany({
      where: {
        ...
        arenaState: "ARENA",
        id:
          activeMascotIds.size > 0
            ? { notIn: [...activeMascotIds] }
            : undefined,
        arenaTeamMembers: { none: { team: { status: "ACTIVE" } } },
      },
 ...
 */
export async function getUnseenPvpAttack(teamId: string): Promise<{
  battleId: string;
  attackerName: string;
  happenedAt: Date;
} | null> {
  try {
    ...
    return {
      battleId: battle.id,
      attackerName:
        battle.attackerPlayer?.displayName ??
        battle.attackerPlayer?.ptcglNick ??
        "outro jogador",
...
export async function runBotBattle(
  ...
  const ownerUser = await prisma.player.findUnique({
    where: { id: playerId },
    include: { user: { select: { role: true } } },
  });
  const isAdminPlayer = ["ADMIN", "SUPER_ADMIN"].includes(
    ownerUser?.user.role ?? "",
  );
  ...
  if (isAdminPlayer) {
    ...
    const allMascotsAdmin = new Map(
      [...attackers, ...bot.defenders].map((m) => [m.id, m]),
    );
    return {
      won,
      ...
      botMascots: bot.defenders.map((m) => ({
        id: m.id,
        pokemonId: m.pokemonId,
        name: m.name,
        level: m.level,
  ...
  if (unseenPvp) {
    throw Object.assign(
      new Error(
        `Voce foi atacado por ${unseenPvp.attackerName} antes de iniciar PvE. Veja o resultado do combate e tente novamente.`,
      ),
  ...
  const pveTodayBRT = new Date().toLocaleDateString("sv-SE", {
    timeZone: "America/Sao_Paulo",
  });
  const pveEarnedToday =
    pvePlayerData?.arenaPveCoinsDate === pveTodayBRT
      ? (pvePlayerData.arenaPveCoinsEarned ?? 0)
      : 0;
  const pveDailyCoinsCap = await getActiveArenaDailyZcLimit();
  if (pveEarnedToday >= pveDailyCoinsCap) {
    throw new Error(
      `Você atingiu o limite diário de ${pveDailyCoinsCap} ZC PvE. O limite reseta à meia-noite (horário de Brasília).`,
    );
  }

  const teamDebuffPct = getArenaDebuffPct(team.enteredAt);
  let attackers = team.members.map((m) =>
    toArenaMascot({ ...m.mascot, combatRole: m.combatRole }, teamDebuffPct),
  );
  attackers = applyBondModifiersToArenaMascots(
    attackers,
    await getBondCombatModifier(attackers.map((m) => m.id)),
  );

  // Bot determinístico: usa pendingBotJson se disponível
  const useDifficulty =
    (team.pendingBotDifficulty as ArenaDifficulty | null) ?? difficulty;
  ...
  if (team.pendingBotJson) {
    ...
    const locked = team.pendingBotJson as {
      botName: string;
  ...
  const baseReward = won
    ? botReward(band.min, band.max, useDifficulty)
    : {
        coins: 0,
        exp: 0,
        food: 0,
  ...
  const reward: ArenaLootFull = won
    ? {
        coins: isCasualPlayer
          ? 0
          : Math.round(baseReward.coins * diff.rewardMult * PVE_REWARD_MULT),
        exp: isCasualPlayer
          ? 0
          : Math.round(baseReward.exp * diff.rewardMult * PVE_REWARD_MULT),
  ...
  const injuredMascotIds =
    defeatedInCombat.length > 0
      ? defeatedInCombat
      : !won && Math.random() < injuryChance
        ? [pick(attackers).id]
        : [];
  ...
  const defeatPreserved = defeatSplit
    ? remainingLoot(currentVault, defeatSplit.stolen, defeatSplit.burned)
    : null;
  ...
  await prisma.$transaction(async (tx) => {
      ...
      data: {
        ...
        lootResult: {
          ...(teamDefeated
            ? {
                reward,
                preserved: defeatPreserved,
                burned: defeatSplit?.burned,
                stolenByBot: defeatSplit?.stolen,
                teamDefeated: true,
              }
            : reward),
          replayMascots: [
    ...
    if (teamDefeated && defeatPreserved) {
      await creditArenaLoot(
        tx,
        playerId,
        defeatPreserved,
        `Cofre restante Arena Z apos K.O. total contra ${botName}`,
        pveDailyCoinsCap,
      );
    ...
    if (won) {
      const syncTicketDrop = await maybeDropSyncTicket(
        tx,
        playerId,
        "arena-pve",
      );
        ...
        await tx.mascotEvent
          .create({
            data: {
              ...
              emoji: "DS",
              description:
                "Encontrou uma metade de ticket do Desafio Sincronizado na Arena Z.",
            },
          })
          .catch(() => null);
      }
  ...
  if (!isCasualPlayer && teamDefeated && defeatPreserved)
    await distributeArenaExp(defeatExpMascotIds, defeatPreserved.exp);
  ...
  return {
    ...
    battleAnimation: (() => {
      const allMascots = new Map(
        [...attackers, ...defenders].map((m) => [m.id, m]),
      );
      return combat.log
        .filter(
          (t) =>
...
export async function lockBotForTeam(
  playerId: string,
  teamId: string,
  difficulty: ArenaDifficulty = "normal",
) {
  ...
  if (unseenPvp) {
    throw Object.assign(
      new Error(
        `Voce foi atacado por ${unseenPvp.attackerName} antes deste combate. Visualize o resultado e tente novamente.`,
      ),
      { blockedByUnseenPvp: true, unseenPvp },
    );
  }
  ...
  await prisma.arenaTeam.update({
    ...
    data: {
      pendingBotJson:
        botJson as unknown as import("@prisma/client").Prisma.InputJsonValue,
...
export async function runOpportunisticAttack(
  ...
  if (targetMascot.arenaState !== "INJURED")
    throw new Error("Mascote nao esta ferido.");
  ...
  if (!rivalRelation)
    throw new Error(
      "Seu time nao tem rivais com este mascote. Apenas rivais podem atacar mascotes feridos.",
    );
  ...
  if (!attackerTeam)
    throw new Error(
      "Voce precisa ter uma equipe ativa na Arena Z para receber a EXP roubada.",
    );
  const equippedMascot = attackerPlayer.mascots[0];
  if (!equippedMascot)
    throw new Error(
      "Equipe um mascote antes de atacar para que ele receba a EXP roubada.",
    );
  const equippedMascotName =
    equippedMascot.nickname ?? getPokemonName(equippedMascot.pokemonId);
  ...
  if (recentOpportunistic)
    throw new Error("Voce ja atacou este mascote neste periodo de ferimento.");
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.arenaBattle.create({
      data: {
        ...
        turnLog: [
          {
            turn: 1,
            actorName: `${attackerPlayer.displayName} (oportunista)`,
            damage: stolenExp,
            action: "ATTACK",
          },
    ...
    const newRest = new Date(
      Math.max(currentRest.getTime(), Date.now()) + extraRestMs,
    );
    await tx.mascot.update({
      where: { id: targetMascotId },
...
export function formatTurnLog(log: PartialTurnLog[]): string[] {
  return log.map((turn) => {
    ...
    const targetIsBot =
      turn.targetOwnerId === null || turn.targetOwnerId === undefined;
...
function rollPvpEggDrop(
  scenario:
    ...
    | "defense"
    | "perfect_defense",
): import("@prisma/client").EggType | null {
  const tables: Record<
    typeof scenario,
    { type: import("@prisma/client").EggType; chance: number }[]
  > = {
    easy: [
      { type: "COMMON", chance: 0.05 },
      { type: "RARE", chance: 0.0025 },
    ],
    balanced: [
      { type: "COMMON", chance: 0.09 },
      { type: "RARE", chance: 0.004 },
      { type: "SPECIAL", chance: 0.0025 },
    ],
    hard: [
      { type: "COMMON", chance: 0.14 },
      { type: "RARE", chance: 0.0075 },
      { type: "SPECIAL", chance: 0.004 },
      { type: "EVENT", chance: 0.0025 },
    ],
    rival: [
      { type: "COMMON", chance: 0.16 },
      { type: "RARE", chance: 0.009 },
      { type: "SPECIAL", chance: 0.005 },
      { type: "EVENT", chance: 0.0025 },
    ],
    defense: [
      { type: "COMMON", chance: 0.08 },
      { type: "RARE", chance: 0.0035 },
      { type: "SPECIAL", chance: 0.0025 },
    ],
    perfect_defense: [
      { type: "COMMON", chance: 0.125 },
      { type: "RARE", chance: 0.006 },
      { type: "SPECIAL", chance: 0.0035 },
      { type: "EVENT", chance: 0.0025 },
    ],
  };
...
export async function runPvpBattle(
  ...
  if (attackTeamId === defenseTeamId)
    throw new Error("Escolha duas equipes diferentes.");

  const mascotBattleSelect = {
    id: true,
    playerId: true,
    pokemonId: true,
    nickname: true,
  ...
  const [attackTeam, defenseTeam] = await Promise.all([
    prisma.arenaTeam.findUnique({
      ...
      include: {
        members: {
          include: { mascot: { select: mascotBattleSelect } },
          orderBy: { slot: "asc" },
        },
        player: { select: { id: true, displayName: true } },
      },
  ...
  if (!attackTeam || attackTeam.playerId !== playerId)
    throw new Error("Equipe atacante nao encontrada.");
  ...
  if (!isTrainingBattle) {
    ...
    if (attackTeam.isTraining || defenseTeam.isTraining) {
      throw new Error(
        "Equipes de treinamento só podem combater entre si. Ambas as equipes precisam ser de treino.",
      );
    }
    if (defenseTeam.playerId === playerId)
      throw new Error("PvP precisa desafiar uma equipe de outro jogador.");
  }
  ...
  if (!isTrainingBattle) {
    ...
    if (atkRoom > defRoom) {
      throw new Error(
        `Sua equipe está na Sala ${atkRoom} e não pode atacar a Sala ${defRoom}. Equipes de salas mais altas só podem ser atacadas por salas iguais ou inferiores.`,
      );
    }
  }
  ...
  const [attackUser, defenseUser] = await Promise.all([
    prisma.player.findUnique({
      where: { id: attackTeam.playerId },
      include: { user: { select: { role: true } } },
    }),
    prisma.player.findUnique({
      where: { id: defenseTeam.playerId },
      include: { user: { select: { role: true } } },
    }),
  ]);
  if (!isTrainingBattle) {
    const isAdminBattle =
      ["ADMIN", "SUPER_ADMIN"].includes(attackUser?.user.role ?? "") ||
      ["ADMIN", "SUPER_ADMIN"].includes(defenseUser?.user.role ?? "");
    if (isAdminBattle)
      throw new Error(
        "Contas admin nao participam de PvP real. Use o modo debug para testes.",
      );
  }
  if (attackTeam.status !== "ACTIVE" || defenseTeam.status !== "ACTIVE")
    throw new Error("As duas equipes precisam estar ativas.");
  if (attackTeam.members.length === 0 || defenseTeam.members.length === 0)
    throw new Error("As equipes precisam ter mascotes.");

  // Batalha casual: sem cooldown nem movimentação de cofre quando qualquer jogador está no Modo Casual
  const isCasualBattle =
    !isTrainingBattle && !!(attackUser?.casualMode || defenseUser?.casualMode);
  ...
  if (!isTrainingBattle && !isCasualBattle) {
    ...
    if (lastVsSameTeam) {
      ...
      if (elapsed < 30 * 60_000) {
        throw new Error(
          "Aguarde 30 min antes de atacar o mesmo time novamente.",
        );
      }
    }
  }
  ...
  let attackers = attackTeam.members.map((m) =>
    toArenaMascot({ ...m.mascot, combatRole: m.combatRole }, attackDebuffPct),
  );
  let defenders = defenseTeam.members.map((m) =>
    toArenaMascot({ ...m.mascot, combatRole: m.combatRole }, defenseDebuffPct),
  );
  attackers = applyBondModifiersToArenaMascots(
    attackers,
    await getBondCombatModifier(attackers.map((m) => m.id)),
  );
  defenders = applyBondModifiersToArenaMascots(
    defenders,
    await getBondCombatModifier(defenders.map((m) => m.id)),
  );
  const combat = runArenaCombat(attackers, defenders);
  const attackerWon = combat.result === "ATTACKER_WIN";
  const defenderWon = combat.result === "DEFENDER_WIN";
  const draw = combat.result === "DRAW";
  const winnerTeam = attackerWon
    ? attackTeam
    : defenderWon
      ? defenseTeam
      : null;
  ...
  const isRival = await prisma.mascotRelation
    .findFirst({
      where: {
        relationshipScore: { lte: -15 },
        OR: [
          {
            mascotA: { playerId: attackTeam.playerId },
            mascotB: { playerId: defenseTeam.playerId },
          },
          {
            mascotA: { playerId: defenseTeam.playerId },
            mascotB: { playerId: attackTeam.playerId },
          },
        ],
      },
      select: { id: true },
  ...
  const defenseScenario: "defense" | "perfect_defense" = perfectDefense
    ? "perfect_defense"
    : "defense";

  // Drops de ovo: apenas em batalhas não-treino
  const attackerEgg =
    !isTrainingBattle && attackerWon ? rollPvpEggDrop(attackScenario) : null;
  const defenderEgg =
    !isTrainingBattle && defenderWon ? rollPvpEggDrop(defenseScenario) : null;
  ...
  if (defenderWon) {
    ...
    defenseRewardCoins = Math.floor(
      baseMin + Math.random() * (baseMax - baseMin + 1),
    );
  ...
  const injuredMascotIds = loserTeam
    ? combat.defeatedMascotIds.filter((id) =>
        loserTeam.members.some((member) => member.mascotId === id),
      )
    : [];
  ...
  const winnerInjuredIds = winnerTeam
    ? combat.defeatedMascotIds.filter((id) =>
        winnerTeam.members.some((member) => member.mascotId === id),
      )
    : [];
  const loserTeamDefeated =
    !!loserTeam && injuredMascotIds.length >= loserTeam.members.length;
  const losingLootWithMultiplier = loserTeam
    ? applyMultiplierToVault(
        {
          coins: loserTeam.vaultCoins,
          exp: loserTeam.vaultExp,
          food: loserTeam.vaultFood,
          sweet: loserTeam.vaultSweet,
        },
  ...
  const preserved = lootSplit
    ? remainingLoot(losingLoot, lootSplit.stolen, lootSplit.burned)
    : null;
  const loserDefeatExpMascotIds =
    loserTeamDefeated && loserTeam
      ? await getArenaExpMascotIds(
          loserTeam.id,
          loserTeam.playerId,
          loserTeam.enteredAt,
        )
  ...
  await prisma.$transaction(async (tx) => {
    const battle = await tx.arenaBattle.create({
      data: {
        ...
        winnerPlayerId: isTrainingBattle
          ? null
          : (winnerTeam?.playerId ?? null),
    ...
    if (!isTrainingBattle && !isCasualBattle && winnerTeam && lootSplit) {
      ...
      await dropArenaGroundSpoils(
        tx,
        lootSplit.burned,
        loserTeam?.playerId ?? null,
        battle.id,
      );
      foundGroundSpoils = await maybeFindArenaGroundSpoils(
        tx,
        winnerTeam.playerId,
        winnerTeam.id,
      );
    ...
    if (
      !isTrainingBattle &&
      !isCasualBattle &&
      defenseRewardCoins > 0 &&
      defenseTeam.status === "ACTIVE"
    ) {
      await tx.arenaTeam.update({
    ...
    if (!isTrainingBattle) {
      if (attackerEgg) {
        await tx.mascotEgg.create({
          data: {
            playerId: attackTeam.playerId,
            type: attackerEgg,
            origin: `Vitória PvP contra ${defenseTeam.player.displayName}`,
          },
        });
      }
      if (defenderEgg) {
        await tx.mascotEgg.create({
          data: {
            playerId: defenseTeam.playerId,
            type: defenderEgg,
            origin: `Defesa PvP contra ${attackTeam.player.displayName}`,
          },
        });
      }
      if (winnerTeam) {
    ...
    if (!isTrainingBattle && !isCasualBattle && loserTeam && preserved) {
      if (loserTeamDefeated) {
        await creditArenaLoot(
          ...
          loserTeam.playerId,
          preserved,
          "Cofre restante Arena Z apos K.O. total em PvP",
          pveDailyCoinsCap,
        );
  ...
  if (!isTrainingBattle && !isCasualBattle && loserTeamDefeated && preserved)
    await distributeArenaExp(loserDefeatExpMascotIds, preserved.exp);

  // EXP de defesa para mascotes defensores sobreviventes (30 EXP base + 20 extra se perfeita) — não no treino
  if (
    !isTrainingBattle &&
    !isCasualBattle &&
    survivingDefenderMascotIds.length > 0
  ) {
  ...
  return {
    ...
    stolen:
      attackerWon && !isTrainingBattle && !isCasualBattle
        ? (lootSplit?.stolen ?? { coins: 0, exp: 0, food: 0, sweet: 0 })
        : { coins: 0, exp: 0, food: 0, sweet: 0 },
    defenseRewardCoins:
      isTrainingBattle || isCasualBattle ? 0 : defenseRewardCoins,
    defenderEgg: isTrainingBattle ? null : defenderEgg,
    attackerEgg: isTrainingBattle ? null : attackerEgg,
    foundGroundSpoils:
      attackerWon && !isTrainingBattle ? foundGroundSpoils : null,
    ...
    battleAnimation: combat.log
      .filter(
        (t) =>
          t.action === "ATTACK" || t.action === "DEFEND" || t.action === "HEAL",
      )
      .map((t) => ({
        turn: t.turn,
        action: t.action,
...
export async function healMascotSus(playerId: string, mascotId: string) {
  const mascot = await prisma.mascot.findUnique({
    ...
    include: {
      relationsAsA: {
        ...
        include: {
          mascotB: {
            select: { playerId: true, statVitality: true, happiness: true },
          },
        },
      },
      relationsAsB: {
  ...
  if (!mascot || mascot.playerId !== playerId)
    throw new Error("Mascote nao encontrado.");
  ...
  const vitalFactor = Math.min(
    20 * 60_000,
    Math.floor((mascot.statVitality + mascot.happiness) / 5) * 60_000,
  );
  ...
  const effectiveRestMs = Math.max(
    30 * 60_000,
    baseRestMs - bonusReductionMs - vitalFactor - friendReductionMs,
  );
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (bonusReductionMs > 0)
      reductions.push(`${mascot.susRestBonusMinutes}min por Super Amigos`);
    if (vitalFactor > 0)
      reductions.push(
        `${Math.floor(vitalFactor / 60_000)}min por vitalidade/felicidade`,
      );
    if (friendReductionMs > 0)
      reductions.push(
        `${Math.floor(friendReductionMs / 60_000)}min por ${friends.length} amigo(s)`,
      );
    const bonusNote =
      reductions.length > 0 ? ` (reduzido: ${reductions.join(", ")})` : "";
    await tx.mascotEvent.create({
      data: {
        mascotId: mascot.id,
        emoji: "🏥",
...
export async function useSusShield(
  shieldOwnerPlayerId: string,
  targetMascotId: string,
) {
  const targetMascot = await prisma.mascot.findUnique({
    where: { id: targetMascotId },
  ...
  if (!targetMascot) throw new Error("Mascote nao encontrado.");
  if (
    targetMascot.arenaState !== "INJURED" &&
    targetMascot.arenaState !== "RESTING"
  ) {
    throw new Error("Mascote nao esta ferido nem em repouso.");
  }
  if (targetMascot.playerId === shieldOwnerPlayerId)
    throw new Error("Nao pode usar o escudo em si mesmo.");
  ...
  const friendship = await prisma.mascotRelation.findFirst({
    where: {
      ...
      OR: [
        {
          mascotA: { playerId: shieldOwnerPlayerId },
          mascotB: { playerId: targetMascot.playerId },
        },
        {
          mascotB: { playerId: shieldOwnerPlayerId },
          mascotA: { playerId: targetMascot.playerId },
        },
      ],
    },
  });
  if (!friendship)
    throw new Error("Voce nao tem amizade com o dono deste mascote.");
  ...
  if (shieldPlayer?.susShieldDate === todayBRT) {
    throw new Error(
      "Voce ja usou seu escudo diario. O escudo reseta a meia-noite (horario de Brasilia).",
    );
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await recordPlayerActivity(tx, {
      ...
      metadata: {
        targetPlayerId: targetMascot.playerId,
        targetPlayerName: targetMascot.player.displayName,
      },
    });
  });
...
export async function claimArenaTutorialBonus(
  playerId: string,
): Promise<{ error?: string; claimed?: boolean }> {
  try {
    const player = await prisma.player.findUnique({
      where: { id: playerId },
      select: { arenaTutorialClaimed: true },
    });
    ...
    const [proteinItem, waterItem] = await Promise.all([
      prisma.shopItem.findFirst({
        where: { type: "MASCOT_BUFF_STAT", active: true },
        select: { id: true },
      }),
      prisma.shopItem.findFirst({
        where: { type: "MASCOT_BUFF_MOOD", active: true },
        select: { id: true },
      }),
    ]);
    await prisma.$transaction(async (tx) => {
      await tx.player.update({
        where: { id: playerId },
        data: { arenaTutorialClaimed: true },
      });
      await tx.zikaCoinWallet.upsert({
        where: { playerId },
        update: {
          balance: { increment: 200 },
          totalEarned: { increment: 200 },
        },
        create: { playerId, balance: 200, totalEarned: 200 },
      });
      await tx.mascotFoodItem.upsert({
        where: { playerId_type: { playerId, type: "FOOD" } },
        update: { quantity: { increment: 3 } },
        create: { playerId, type: "FOOD", quantity: 3 },
      });
      ...
      if (proteinItem)
        await tx.playerInventory.upsert({
          where: { playerId_itemId: { playerId, itemId: proteinItem.id } },
          update: { quantity: { increment: 1 } },
          create: { playerId, itemId: proteinItem.id, quantity: 1 },
        });
      if (waterItem)
        await tx.playerInventory.upsert({
          where: { playerId_itemId: { playerId, itemId: waterItem.id } },
          update: { quantity: { increment: 1 } },
          create: { playerId, itemId: waterItem.id, quantity: 1 },
        });
    });
    return { claimed: true };
  } catch (err) {
...
export async function adminSetMascotArenaState(
  mascotId: string,
  state: "FREE" | "INJURED" | "RESTING",
) {
  const restingUntil =
    state === "RESTING"
      ? new Date(Date.now() + ARENA_Z_CONFIG.restAfterSusHours * 3_600_000)
      : null;
...
export async function cleanupExpiredArenaResting(
...
export async function adminRepairArenaStates(targetPlayerId?: string): Promise<{
  ...
  if (arenaButNoTeam.length > 0) {
    ...
    for (const m of arenaButNoTeam) {
      details.push(
        `[ARENA→FREE] ${m.nickname ?? `#${m.pokemonId}`} (player ${m.playerId})`,
      );
    }
  }
  ...
  if (expiredResting.length > 0) {
    ...
    for (const m of expiredResting) {
      details.push(
        `[RESTING_EXPIRED→FREE] ${m.nickname ?? `#${m.pokemonId}`} (player ${m.playerId})`,
      );
    }
  }
  ...
  const mismatchedMembers = await prisma.arenaTeamMember.findMany({
    ...
    include: {
      mascot: {
        select: { id: true, nickname: true, pokemonId: true, arenaState: true },
      },
  ...
  for (const member of mismatchedMembers) {
    if (member.mascot.arenaState === "FREE") {
      ...
      details.push(
        `[FREE→ARENA mismatch fixed] ${member.mascot.nickname ?? `#${member.mascot.pokemonId}`} na equipe ${member.team.name}`,
      );
    ...
    if (
      ...
      details.push(
        `[MEMBER_REMOVED injured/resting] ${member.mascot.nickname ?? `#${member.mascot.pokemonId}`} da equipe ${member.team.name}`,
      );
      fixedIds.push(member.mascot.id);
    }
  }
  ...
  if (nowEmptyAfterFix.length > 0) {
    ...
    for (const t of nowEmptyAfterFix)
      details.push(`[EMPTY_AFTER_FIX_DELETED] ${t.name}`);
  }
