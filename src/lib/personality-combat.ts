...
export function personalityOffenseMult(
  actor: CombatUnit,
  target: CombatUnit,
  ...
  switch (actor.personality) {
    case "PROUD":
      return hpPct > 0.7 ? 1.06 : 1;
    case "DRAMATIC":
      return hpPct < 0.35 ? 1.1 : 1;
    case "SERENE":
      return 0.96;
    case "COMPETITIVE":
      return combatStatTotal(target) > combatStatTotal(actor) ? 1.07 : 1;
    case "CURIOUS":
      return combatStatTotal(target) >= combatStatTotal(actor) ? 1.05 : 1;
    case "TIMID":
      return hitTaken.has(actor.id) ? 1.05 : 1;
    case "CHAOTIC":
      return 0.92 + Math.random() * 0.2;
    case "GLUTTON":
      return 0.96;
    default:
      return 1;
...
export function personalityDefenseMult(
  target: CombatUnit,
  hp: Map<string, number>,
  hitTaken: Set<string>,
) {
  const hpPct = (hp.get(target.id) ?? 0) / Math.max(1, target.hp);
  if (target.personality === "LAZY" && hpPct > 0.5) return 0.92;
  if (target.personality === "TIMID" && !hitTaken.has(target.id)) return 0.9;
  return 1;
}
...
export function debuffResistanceFactor(source: CombatUnit, target: CombatUnit) {
  ...
  const resist =
    target.instinct * DEBUFF_RESISTANCE.targetInstinctWeight +
    target.vitality * DEBUFF_RESISTANCE.targetVitalityWeight;
  const ratio = power / (power + Math.max(1, resist));
  const factor =
    DEBUFF_RESISTANCE.minEffect +
    (DEBUFF_RESISTANCE.maxEffect - DEBUFF_RESISTANCE.minEffect) * ratio;
  const clamped = Math.max(
    DEBUFF_RESISTANCE.minEffect,
    Math.min(DEBUFF_RESISTANCE.maxEffect, factor),
  );
...
export function rollPlayfulTeamBuff(
  team: CombatUnit[],
  rng: () => number = Math.random,
) {
  return team
    .filter((m) => m.personality === "PLAYFUL")
    .some(() => rng() < 0.5);
}

// Multiplicador de agilidade efetiva por personalidade (para ações extras).
export function personalityAgilityMult(
  actor: CombatUnit,
  round: number,
  playfulBuffActive: boolean,
) {
  let mult = 1;
...
export function rollTravessoDebuff(
  actor: CombatUnit,
  target: CombatUnit,
  rng: () => number = Math.random,
) {
  ...
  const stat =
    aff && (debuffable as readonly string[]).includes(aff)
      ? (aff as (typeof debuffable)[number])
      : debuffable.reduce(
          (best, s) => (target[s] > target[best] ? s : best),
          "force",
        );
