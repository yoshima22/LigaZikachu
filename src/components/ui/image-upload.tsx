...
export function ImageUpload({
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (sizeMb > maxMb) {
      setWarning(`Arquivo grande (${sizeMb.toFixed(1)}MB). Máximo recomendado: ${maxMb}MB. Tente usar uma URL externa.`);
      // Continua mesmo assim — o limite do servidor é 12MB
    }

    const reader = new FileReader();
    reader.onload = () => onChange(reader.result as string);
    reader.onerror = () => setWarning("Erro ao ler o arquivo. Tente novamente.");
