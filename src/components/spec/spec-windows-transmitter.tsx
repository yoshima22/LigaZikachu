...
export function SpecWindowsTransmitter({ streamId }: { streamId: string }) {
  ...
  useEffect(() => {
    if (connected) return;
    const check = () => { void getWindowsTransmitterPairingStateAction(streamId).then((state) => { if (state.connected) setConnected({ deviceName: state.deviceName, processName: state.processName, resolution: state.resolution, fps: state.fps, quality: state.quality }); }); };
    check();
