...
export async function POST(request: Request) {
  const body = await request.json().catch(() => ({})) as { code?: string; deviceName?: string; processName?: string };
  ...
  await prisma.specSignal.update({ where: { id: row.id }, data: { kind: "WIN_CONNECTED", payload: { tokenHash, deviceName: String(body.deviceName ?? "Windows").slice(0, 80), processName: String(body.processName ?? "").slice(0, 120), connectedAt: new Date().toISOString() } as Prisma.InputJsonValue } });
