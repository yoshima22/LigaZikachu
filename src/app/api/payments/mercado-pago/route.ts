...
export async function POST(req:NextRequest){
  ...
  const order=await prisma.ligaCashOrder.findUnique({where:{id:orderId}});
  if(!order||Math.round(Number(payment.transaction_amount)*100)!==order.amountCents)return NextResponse.json({ok:true});
  if(order.provider==="MERCADO_PAGO_CARD"&&payment.payment_type_id!=="credit_card")return NextResponse.json({ok:true});
  if(order.provider==="MERCADO_PAGO"&&payment.payment_method_id!=="pix")return NextResponse.json({ok:true});
  if(!order.providerPaymentId){
    await prisma.ligaCashOrder.updateMany({where:{id:order.id,providerPaymentId:null},data:{providerPaymentId:String(payment.id)}});
  }
  const linked=await prisma.ligaCashOrder.findUnique({where:{id:order.id},select:{providerPaymentId:true}});
  if(linked?.providerPaymentId!==String(payment.id))return NextResponse.json({ok:true});
  if(payment.status==="approved")await fulfillLigaCashOrder(order.id,String(payment.id));
  else if(["refunded","charged_back"].includes(payment.status))await refundLigaCashOrder(order.id,String(payment.id),payment.status==="charged_back"?"CHARGEBACK":"REFUND");
