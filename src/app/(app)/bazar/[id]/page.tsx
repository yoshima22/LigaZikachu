...
interface ListingDetail {
  ...
  player: {
    ...
    itemWishlist: Array<{
      itemId: string;
      item: { name: string; type: string; rarity: string; imageUrl: string | null; description: string | null };
    }>;
  };
  ...
  const bundleTitle = bundleItems.length === 1
    ...
    : bundleItems.length > 1
      ? (bundleItems.every((item) => item.mascotId) ? `Pacote com ${bundleItems.length} mascotes`
        : bundleItems.every((item) => !item.mascotId) ? `Pacote com ${bundleItems.length} itens diferentes`
        : `Pacote misto com ${bundleItems.filter((item) => item.mascotId).length} mascote(s) e ${bundleItems.filter((item) => !item.mascotId).length} item(ns)`)
      : "";
      ...
      <div className={`rounded-2xl border bg-slate-950/60 overflow-hidden ${isPremium ? "border-amber-300/60 shadow-[0_0_30px_rgba(250,204,21,0.10)]" : "border-border"}`}>
        ...
        <div className="flex items-center justify-center bg-slate-900/80 h-48">
          {bundleItems.length > 0 ? (
            <div className="flex flex-col items-center gap-2 text-amber-200"><Gavel size={54}/><span className="text-sm font-black">{bundleTitle}</span><span className="text-xs text-slate-400">{bundleItems.reduce((sum, item) => sum + Math.max(1, item.quantity || 1), 0)} ativos no total</span></div>
          ) : isMascot && pokemonId ? (
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                {bundleItems.length > 0 ? bundleTitle : isMascot ? mascotFullName : (displayName ?? itemType ?? "Item")}
            ...
            <div className="rounded-xl border border-[#FFCB05]/20 bg-[#FFCB05]/5 p-3">
              <div className="mb-3 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500">Mascotes e itens que o vendedor procura receber.</p>
                </div>
              </div>
              {listing.player.pokemonWishlist.length + listing.player.itemWishlist.length > 0 ? (
                <div className="space-y-3">
                  {listing.player.pokemonWishlist.length > 0 && <div>
                    <p className="mb-1.5 text-[10px] font-bold uppercase tracking-wide text-slate-500">Mascotes</p>
                  ...
                  {listing.player.itemWishlist.length > 0 && <div>
                    <p className="mb-1.5 text-[10px] font-bold uppercase tracking-wide text-slate-500">Itens</p>
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                      {listing.player.itemWishlist.map(({ itemId, item }) => (
                        <div key={itemId} className="flex items-center gap-2 rounded-lg border border-cyan-400/20 bg-slate-950/60 px-2 py-1.5" title={item.description ?? item.name}>
                          <span className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden text-lg">
                            {item.imageUrl ? (
                              // eslint-disable-next-line @next/next/no-img-element
                          ...
                          <span className="min-w-0">
                            <span className="block truncate text-[11px] font-semibold text-slate-200">{item.name}</span>
                            <span className="text-[9px] uppercase text-cyan-300/70">{item.rarity}</span>
                          </span>
                        </div>
                      ))}
                    </div>
