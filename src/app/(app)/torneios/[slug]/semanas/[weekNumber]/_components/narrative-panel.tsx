...
export function NarrativePanel({ weekId, weekLabel, existingNarrative, generatedAt, isAdmin }: Props) {
  ...
  const handleGenerate = () => {
    const label = text ? "Regenerar o recap narrativo desta semana? O texto atual será substituído." : "Gerar o recap narrativo desta semana via IA?";
    ...
    <div className="rounded-2xl border border-border bg-slate-950/50 p-5 space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="font-semibold text-slate-200">Recap Narrativo</h2>
            <p className="mt-0.5 text-xs text-slate-500">
              Texto gerado por IA com análise da semana, resultados e impactos na liga.
            </p>
          </div>
        </div>
      ...
      {pending && (
        <div className="flex items-center gap-3 rounded-xl border border-[#FFCB05]/20 bg-[#FFCB05]/5 px-4 py-5 text-sm text-slate-400">
          ...
          <div>
            <p className="font-medium text-slate-200">Professor Enguiça está analisando a semana…</p>
      ...
      {!pending && text ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-dashed border-border px-4 py-5 text-xs text-slate-500">
          ...
          <div>
            <p className="text-slate-400">Nenhum recap gerado ainda para esta semana.</p>
