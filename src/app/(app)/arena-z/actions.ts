...
/** Coleta manualmente um espólio do chão da arena (bônus extra além da coleta automática PvP) */
export async function collectGroundSpoilAction(spoilId: string): Promise<{ error?: string; loot?: { coins: number; exp: number; food: number; sweet: number } }> {
  try {
    const playerId = await getCurrentPlayerId();

    // Busca o espólio e valida
    const spoil = await prisma.arenaGroundSpoil.findUnique({ where: { id: spoilId } });
    if (!spoil) return { error: "Espólio não encontrado." };
    if (spoil.status !== "AVAILABLE") return { error: "Este espólio já foi coletado." };
    if (spoil.sourcePlayerId === playerId) return { error: "Você não pode coletar seu próprio espólio." };

    // Verifica que o jogador tem ao menos uma equipe ativa na arena (precisa estar "no campo")
    const activeTeam = await prisma.arenaTeam.findFirst({
      where: { playerId, status: "ACTIVE" },
      select: { id: true },
    });
    if (!activeTeam) return { error: "Você precisa ter uma equipe ativa na arena para coletar espólios." };

    await prisma.$transaction(async (tx) => {
      await tx.arenaGroundSpoil.update({
        where: { id: spoilId, status: "AVAILABLE" },
        data: { status: "CLAIMED", foundByPlayerId: playerId, claimedAt: new Date() },
      });
      if (spoil.coins > 0) {
        const { creditCoins } = await import("@/lib/zikacoins");
        await creditCoins(tx, { playerId, type: "BET_WON", amount: spoil.coins, description: "Espólio do chão da Arena Z" });
      }
      if (spoil.food > 0) {
        await tx.mascotFoodItem.upsert({
          where: { playerId_type: { playerId, type: "FOOD" } },
          update: { quantity: { increment: spoil.food } },
          create: { playerId, type: "FOOD", quantity: spoil.food },
        });
      }
      if (spoil.sweet > 0) {
        await tx.mascotFoodItem.upsert({
          where: { playerId_type: { playerId, type: "SWEET" } },
          update: { quantity: { increment: spoil.sweet } },
          create: { playerId, type: "SWEET", quantity: spoil.sweet },
        });
      }
    });

    revalidatePath("/arena-z");
    return { loot: { coins: spoil.coins, exp: spoil.exp, food: spoil.food, sweet: spoil.sweet } };
  } catch (err) {
    return { error: err instanceof Error ? err.message : "Erro ao coletar espólio." };
  }
}
