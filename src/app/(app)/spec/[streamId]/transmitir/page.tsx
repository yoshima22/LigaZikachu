...
export default async function SpecBroadcastPage({ params }: { params: Promise<{ streamId: string }> }) {
  ...
  const stream = config.enabled ? await prisma.specStream.findUnique({
    where: { id: streamId },
    select: { id: true, matchId: true, tournamentId: true, title: true, broadcasterUserId: true, status: true, provider: true, youtubeVideoId: true },
  }).catch(() => null) : null;
  ...
  const windowsTransmitterConnected = Boolean(await prisma.specSignal.findFirst({ where: { streamId: stream.id, fromUserId: session.user.id, kind: "WIN_CONNECTED" }, select: { id: true } }));
