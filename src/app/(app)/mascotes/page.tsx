  ...
  const [
    ...
    prisma.mascot.findMany({
      ...
      select: {
        ...
        isEquipped: true, isFavorite: true, isShiny: true, evolutionLocked: true, expLocked: true, operationsLocked: true, primordialBoundPlayerId: true,
...
export default async function MascotesPage() {
  ...
  const repairedRainbowResets = await repairRecentRainbowFeatherDevolutions(player.id).catch((error) => {
  ...
  const mascotData = featuredMascots.map(m => ({
    ...
    primordialBoundPlayerId: m.primordialBoundPlayerId,
    isShiny: m.isShiny,
    ivRating: m.ivRating,
    ivScore: m.ivScore,
