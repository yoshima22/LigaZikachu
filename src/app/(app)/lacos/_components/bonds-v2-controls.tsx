...
import { BONDS_V2_BALANCE, REFUGE_LOCATIONS, type RefugeLocation } from "@/lib/mascot-bonds-v2";
...
export function RefugeLocationScene({ location, occupants, ownMascots, backgroundUrl, stories }: { location: RefugeLocation; occupants: SceneMascot[]; ownMascots: SceneMascot[]; backgroundUrl: string; stories: SceneStory[] }) {
  ...
  const storyPages = Math.max(1, Math.ceil(stories.length / storiesPerPage));
  const safeStoryPage = Math.min(storyPage, storyPages);
  const visibleStories = stories.slice((safeStoryPage - 1) * storiesPerPage, safeStoryPage * storiesPerPage);
  ...
  return <article className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950 shadow-2xl shadow-black/20">
    ...
    <div className="border-t border-white/10 bg-[#050914] p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-fuchsia-300">Eventos desta região</p><p className="mt-1 text-xs text-slate-400">O diário continua sendo preenchido pelo mundo a cada ciclo, mesmo sem ninguém abrir esta página.</p></div><span className="rounded-full bg-white/5 px-2 py-1 text-[9px] text-slate-500">{stories.length} relatos preservados</span></div>{stories.length === 0 ? <p className="rounded-xl border border-dashed border-white/10 p-4 text-xs text-slate-500">Ainda não há histórias registradas neste local.</p> : <><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{visibleStories.map((story) => <div key={story.id} className={`rounded-xl border p-3 ${story.conflict ? "border-rose-400/30 bg-rose-400/[.08]" : "border-emerald-400/20 bg-emerald-400/[.06]"}`}><p className={`text-[9px] font-black uppercase tracking-wider ${story.conflict ? "text-rose-300" : "text-emerald-300"}`}>{story.conflict ? "⚡ Conflito percebido" : "✦ Momento social"} · {story.when}</p><p className="mt-1 text-xs font-bold text-white">{story.title}</p><p className="mt-1 text-[10px] leading-4 text-slate-300">{story.description}</p></div>)}</div>{storyPages > 1 && <div className="mt-3 flex items-center justify-center gap-3"><button type="button" disabled={safeStoryPage <= 1} onClick={() => setStoryPage(safeStoryPage - 1)} className="rounded-lg border border-white/10 p-2 text-slate-300 disabled:opacity-25"><ChevronLeft size={14} /></button><span className="text-[10px] text-slate-500">Relatos {((safeStoryPage - 1) * storiesPerPage) + 1}–{Math.min(stories.length, safeStoryPage * storiesPerPage)} de {stories.length}</span><button type="button" disabled={safeStoryPage >= storyPages} onClick={() => setStoryPage(safeStoryPage + 1)} className="rounded-lg border border-white/10 p-2 text-slate-300 disabled:opacity-25"><ChevronRight size={14} /></button></div>}</>}</div>
  </article>;
}
