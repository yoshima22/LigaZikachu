...
export type SceneMascot = { id: string; name: string; sprite: string; level: number; personality: string; owner: string; own: boolean; location: string | null };
export type SceneStory = { id: string; title: string; description: string; conflict: boolean; when: string };
...
export function RefugeLocationScene({ location, occupants, ownMascots, backgroundUrl, stories }: { location: RefugeLocation; occupants: SceneMascot[]; ownMascots: SceneMascot[]; backgroundUrl: string; stories: SceneStory[] }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950 shadow-2xl shadow-black/20">
    ...
    <div className="border-t border-white/10 bg-[#050914] p-4"><div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-fuchsia-300">Eventos desta região</p><p className="mt-1 text-xs text-slate-400">Relatos explicam encontros, cooperações e conflitos que alteraram os Laços.</p></div><span className="rounded-full bg-white/5 px-2 py-1 text-[9px] text-slate-500">ambiência recente</span></div>{stories.length === 0 ? <p className="rounded-xl border border-dashed border-white/10 p-4 text-xs text-slate-500">Ainda não há histórias registradas neste local.</p> : <div className="grid gap-2 sm:grid-cols-3">{stories.map((story) => <div key={story.id} className={`rounded-xl border p-3 ${story.conflict ? "border-rose-400/30 bg-rose-400/[.08]" : "border-emerald-400/20 bg-emerald-400/[.06]"}`}><p className={`text-[9px] font-black uppercase tracking-wider ${story.conflict ? "text-rose-300" : "text-emerald-300"}`}>{story.conflict ? "⚡ Conflito percebido" : "✦ Momento social"} · {story.when}</p><p className="mt-1 text-xs font-bold text-white">{story.title}</p><p className="mt-1 text-[10px] leading-4 text-slate-300">{story.description}</p></div>)}</div>}</div>
  </article>;
}
