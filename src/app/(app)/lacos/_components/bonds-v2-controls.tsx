...
export function BondsTutorial() {
  ...
  useEffect(() => { if (!window.localStorage.getItem("lacos-v2-guia-visto")) setOpen(true); }, []);
  function close() { window.localStorage.setItem("lacos-v2-guia-visto", "1"); setOpen(false); }
  const pages = [
    { title: "Bem-vindo ao novo Refúgio", eyebrow: "Do sistema antigo para um mundo vivo", body: <div><p className="text-sm leading-6 text-slate-300">Antes, Laços eram apenas uma lista de relações. Agora seus mascotes ocupam espaços públicos, encontram criaturas de outros treinadores, produzem recursos e criam histórias mesmo quando ninguém está com a página aberta.</p><div className="mt-5 grid gap-3 md:grid-cols-2">{[
    ...
    ].map(([title, text]) => <div key={title} className="rounded-2xl border border-white/10 bg-white/[.025] p-4"><h3 className="font-bold text-white">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{text}</p></div>)}</div></div> },
    { title: "Explore o Refúgio", eyebrow: "Aba 1 · rotinas públicas", body: <div><div className="grid gap-4 md:grid-cols-2">{Object.entries(REFUGE_LOCATIONS).map(([id, location]) => <article key={id} className="rounded-2xl border border-white/10 bg-white/[.025] p-4"><h3 className="font-black text-white">{location.icon} {location.label}</h3><p className="mt-2 text-xs leading-5 text-slate-300">{location.impact}</p></article>)}</div><p className="mt-4 rounded-xl bg-cyan-300/[.06] p-4 text-sm leading-6 text-slate-300">Use busca, filtros e paginação para ver habitantes e diário. Cada mascote mostra há quanto tempo está no local. A troca tem recarga individual de 2 horas.</p></div> },
    { title: "Momentos importantes", eyebrow: "Aba 2 · suas decisões", body: <div className="space-y-4 text-sm leading-6 text-slate-300"><p>Eventos comuns ficam no diário da região. Somente acontecimentos com impacto real aparecem aqui para você escolher como o treinador intervém.</p><p className="rounded-xl border border-amber-300/15 bg-amber-300/[.05] p-4">As opções mudam conforme local, relação, personalidade e contexto. Algumas consomem itens da Mochila; sem resposta, o mascote pode decidir sozinho sem gastar seus recursos.</p><p>Conflitos não são necessariamente ruins: eles podem criar rivalidades saudáveis, Clubes da Luta e recursos competitivos.</p></div> },
    { title: "Mapa social", eyebrow: "Aba 3 · treinadores e grupos", body: <div className="grid gap-4 md:grid-cols-2"><div className="rounded-2xl border border-emerald-300/15 p-4"><h3 className="font-bold text-emerald-300">Círculos de amizade</h3><p className="mt-2 text-sm leading-6 text-slate-300">Dois ou mais vínculos positivos em torno do mesmo mascote favorecem eventos coletivos e itens comuns de Laços.</p></div><div className="rounded-2xl border border-rose-300/15 p-4"><h3 className="font-bold text-rose-300">Clubes da Luta</h3><p className="mt-2 text-sm leading-6 text-slate-300">Duas ou mais rivalidades favorecem desafios, competição e itens próprios quando se encontram no Campo de Treino.</p></div><p className="md:col-span-2 text-sm leading-6 text-slate-300">Pesquise um treinador e clique no nome para abrir estatísticas, média das relações e todas as duplas ligadas a ele.</p></div> },
    { title: "Laços e efeitos", eyebrow: "Aba 4 · gameplay e gerenciamento", body: <div><div className="mb-4 rounded-2xl border border-cyan-300/15 bg-cyan-300/[.05] p-4 text-sm leading-6 text-slate-300">A escala vai de <strong>−100</strong> a <strong>+100</strong>. Amizades oferecem cooperação; rivalidades oferecem benefícios competitivos. Os efeitos aparecem visualmente nos combates compatíveis.</div><div className="grid gap-3 md:grid-cols-2">{GUIDE_RELATIONS.map((relation) => <article key={relation.name} className={`rounded-2xl border p-4 ${relation.tone}`}><div className="flex justify-between gap-3"><h3 className="font-black">{relation.name}</h3><span className="text-xs">{relation.range}</span></div><p className="mt-2 text-xs leading-5 text-slate-300">{relationEffectV2(relation.score)}</p></article>)}</div><p className="mt-4 text-sm leading-6 text-slate-300"><strong>Afastar não apaga instantaneamente:</strong> inicia um prazo de 24 horas. Ao concluir, o vínculo dos dois lados é removido por completo e nenhuma relação oculta ocupa a vaga.</p></div> },
    { title: "Mochila de Laços", eyebrow: "Aba 5 · recursos e Bazar", body: <div><p className="mb-4 text-sm leading-6 text-slate-300">A Mochila reúne todos os itens sociais, inclusive os que estão zerados. Eles vêm das rotinas, marcos e conflitos; podem ser usados em decisões ou negociados entre jogadores no Bazar.</p><div className="grid gap-2 md:grid-cols-2">{GUIDE_ITEMS.map(([type, name, source, effect]) => <article key={type} className="flex gap-3 rounded-xl border border-white/10 p-3"><span className="text-2xl">{getShopItemEmoji(type)}</span><div><h3 className="text-xs font-bold text-white">{name} <span className="text-violet-300">· {source}</span></h3><p className="mt-1 text-[10px] leading-4 text-slate-400">{effect}</p></div></article>)}</div></div> },
  ];
  const current = pages[page];
  return <><button onClick={() => { setPage(0); setOpen(true); }} className="inline-flex items-center gap-2 rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-xs font-bold text-cyan-100 hover:bg-cyan-300/15"><BookOpen size={15} /> Guia completo do Refúgio</button>{open && <div className="fixed inset-0 z-[100] grid place-items-center bg-black/85 p-3 backdrop-blur-md" onClick={close}><div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-fuchsia-300/25 bg-[#080d1c] shadow-2xl" onClick={(event) => event.stopPropagation()}><header className="flex items-start justify-between border-b border-white/10 p-5"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-fuchsia-300">{current.eyebrow}</p><h2 className="mt-1 text-2xl font-black text-white">{current.title}</h2></div><button onClick={close} className="rounded-full border border-white/10 p-2 text-slate-400 hover:text-white"><X size={17} /></button></header><div className="overflow-y-auto p-5">{current.body}</div><footer className="flex items-center justify-between border-t border-white/10 bg-slate-950/70 p-4"><button disabled={page === 0} onClick={() => setPage((value) => value - 1)} className="rounded-xl border border-white/10 px-4 py-2 text-sm text-white disabled:opacity-30">Anterior</button><div className="text-center"><p className="text-xs text-slate-400">Página {page + 1} de {pages.length}</p><div className="mt-2 flex gap-1">{pages.map((_, index) => <button key={index} onClick={() => setPage(index)} aria-label={`Página ${index + 1}`} className={`h-1.5 rounded-full transition-all ${index === page ? "w-6 bg-fuchsia-400" : "w-2 bg-slate-700"}`} />)}</div></div><button onClick={() => page === pages.length - 1 ? close() : setPage((value) => value + 1)} className="rounded-xl bg-fuchsia-400 px-4 py-2 text-sm font-bold text-slate-950">{page === pages.length - 1 ? "Entendi" : "Próxima"}</button></footer></div></div>}</>;
}

export type BondItem = { id: string; a: string; b: string; owner: string; ownerA: string; ownerB: string; spriteA: string; spriteB: string; score: number; tier: string; effect: string; interactions: number; active: boolean; transitionAt: string | null };
...
export function BondDirectoryV2({ relations }: { relations: BondItem[] }) {
  ...
  return <div className="font-sans">
    ...
    <div className="space-y-3">{visible.length === 0 ? <div className="rounded-xl border border-dashed border-white/10 p-6 text-center text-sm text-slate-500">Nenhum vínculo corresponde aos filtros.</div> : visible.map((relation) => { const distancing = relation.transitionAt !== null && new Date(relation.transitionAt).getTime() > Date.now(); return <article key={relation.id} className={`rounded-2xl border p-4 ${relation.score < -14 ? "border-rose-400/20 bg-rose-400/[.035]" : relation.score > 14 ? "border-emerald-400/20 bg-emerald-400/[.035]" : "border-white/10 bg-white/[.025]"}`}><div className="grid items-center gap-3 sm:grid-cols-[1fr_auto_1fr_auto]"><div className="flex min-w-0 items-center gap-3 rounded-xl border border-cyan-300/15 bg-cyan-300/[.05] p-3"><img src={relation.spriteA} alt="" className="h-12 w-12 shrink-0 rounded-full bg-slate-900 object-contain" /><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-wider text-cyan-300">Mascote do jogador</p><p className="truncate text-base font-semibold text-white">{relation.a}</p><p className="truncate text-xs text-slate-400">Treinador: {relation.ownerA}</p></div></div><div className="text-center"><p className="text-lg text-slate-500">→</p><p className="text-[10px] uppercase text-slate-500">sente por</p></div><div className="flex min-w-0 items-center gap-3 rounded-xl border border-violet-300/15 bg-violet-300/[.05] p-3"><img src={relation.spriteB} alt="" className="h-12 w-12 shrink-0 rounded-full bg-slate-900 object-contain" /><div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-wider text-violet-300">Mascote encontrado</p><p className="truncate text-base font-semibold text-white">{relation.b}</p><p className="truncate text-xs text-slate-400">Treinador: {relation.ownerB}</p></div></div><div className="min-w-[105px] text-right"><p className="font-semibold text-white">{relation.tier}</p><p className="text-sm text-slate-300">{relation.score > 0 ? "+" : ""}{relation.score}</p><p className="text-xs text-slate-500">{relation.interactions} interações</p><span className={`mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${distancing ? "bg-amber-300/10 text-amber-200" : "bg-emerald-300/10 text-emerald-200"}`}>{distancing ? "Afastando" : "Presente"}</span></div></div>{distancing && <p className="mt-2 text-xs font-semibold text-amber-300">Encerramento em {new Date(relation.transitionAt!).toLocaleString("pt-BR", { timeZone: "America/Sao_Paulo", day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</p>}<div className="my-3 rounded-xl border border-white/5 bg-black/20 p-3"><p className="text-xs font-semibold uppercase tracking-wider text-fuchsia-300">Efeito desta relação</p><p className="mt-1 text-sm leading-5 text-slate-300">{relation.effect}</p></div><BondV2Buttons relationId={relation.id} active={relation.active} transitionAt={relation.transitionAt} /></article>; })}</div>
...
export function TrainerBondExplorer({ relations }: { relations: BondItem[] }) {
  ...
  const perPage = 5;
  const trainers = useMemo(() => Object.values(relations.filter((relation) => relation.active).reduce<Record<string, { name: string; relations: BondItem[] }>>((result, relation) => {
    result[relation.ownerB] ??= { name: relation.ownerB, relations: [] };
    result[relation.ownerB].relations.push(relation);
    return result;
  }, {})).filter((trainer) => {
    const matchesQuery = trainer.name.toLocaleLowerCase("pt-BR").includes(query.trim().toLocaleLowerCase("pt-BR"));
    const matchesKind = kind === "ALL" || trainer.relations.some((relation) => kind === "FRIEND" ? relation.score >= 15 : relation.score <= -15);
    return matchesQuery && matchesKind;
  }).sort((a, b) => b.relations.length - a.relations.length), [relations, query, kind]);
  const pages = Math.max(1, Math.ceil(trainers.length / perPage));
  const visible = trainers.slice((Math.min(page, pages) - 1) * perPage, Math.min(page, pages) * perPage);
  const profile = trainers.find((trainer) => trainer.name === selected) ?? null;
  return <div className="space-y-4">
    <div className="grid gap-2 sm:grid-cols-[1fr_180px]"><div className="relative"><Search size={15} className="absolute left-3 top-3 text-slate-500" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); setSelected(null); }} placeholder="Pesquisar treinador..." className="w-full rounded-xl border border-white/10 bg-slate-950 py-2.5 pl-9 pr-3 text-sm text-white outline-none focus:border-cyan-300/40" /></div><select value={kind} onChange={(event) => { setKind(event.target.value as typeof kind); setPage(1); }} className="rounded-xl border border-white/10 bg-slate-950 px-3 text-sm text-slate-200"><option value="ALL">Todas as relações</option><option value="FRIEND">Com amizades</option><option value="RIVAL">Com rivalidades</option></select></div>
    <div className="space-y-2">{visible.length === 0 ? <div className="rounded-xl border border-dashed border-white/10 p-5 text-center text-xs text-slate-500">Nenhum treinador encontrado.</div> : visible.map((trainer) => { const friends = trainer.relations.filter((relation) => relation.score >= 15).length; const rivals = trainer.relations.filter((relation) => relation.score <= -15).length; const average = Math.round(trainer.relations.reduce((total, relation) => total + relation.score, 0) / trainer.relations.length); return <button key={trainer.name} type="button" onClick={() => setSelected(selected === trainer.name ? null : trainer.name)} className={`w-full rounded-xl border p-3 text-left transition ${selected === trainer.name ? "border-cyan-300/40 bg-cyan-300/[.07]" : "border-white/10 bg-black/20 hover:border-white/20"}`}><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-bold text-white">{trainer.name}</p><p className="text-[10px] text-slate-500">{trainer.relations.length} relações · média {average > 0 ? "+" : ""}{average}</p></div><div className="flex gap-2 text-[10px]"><span className="rounded-full bg-emerald-400/10 px-2 py-1 text-emerald-300">{friends} amizades</span><span className="rounded-full bg-rose-400/10 px-2 py-1 text-rose-300">{rivals} rivalidades</span></div></div></button>; })}</div>
    {pages > 1 && <div className="flex items-center justify-between text-xs text-slate-400"><button disabled={page <= 1} onClick={() => setPage((value) => value - 1)} className="rounded-lg border border-white/10 p-2 disabled:opacity-30"><ChevronLeft size={15} /></button><span>Página {Math.min(page, pages)} de {pages}</span><button disabled={page >= pages} onClick={() => setPage((value) => value + 1)} className="rounded-lg border border-white/10 p-2 disabled:opacity-30"><ChevronRight size={15} /></button></div>}
    {profile && <div className="rounded-2xl border border-cyan-300/20 bg-slate-950/70 p-4"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-wider text-cyan-300">Perfil social</p><h3 className="text-lg font-bold text-white">Relações com {profile.name}</h3></div><button onClick={() => setSelected(null)} className="rounded-full border border-white/10 p-1.5 text-slate-400"><X size={14} /></button></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{profile.relations.map((relation) => <article key={relation.id} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.025] p-2"><img src={relation.spriteA} alt="" className="h-9 w-9 rounded-full bg-slate-900 object-contain" /><div className="min-w-0 flex-1"><p className="truncate text-xs text-white">{relation.a} <span className="text-slate-500">com</span> {relation.b}</p><p className="text-[10px] text-slate-400">{relation.tier} · {relation.score > 0 ? "+" : ""}{relation.score} · {relation.interactions} interações</p></div><img src={relation.spriteB} alt="" className="h-9 w-9 rounded-full bg-slate-900 object-contain" /></article>)}</div></div>}
  </div>;
}
...
export function BondInventoryV2({ items }: { items: BondInventoryItem[] }) {
  ...
  return <section className="space-y-4 font-sans">
    <div className="rounded-3xl border border-violet-300/15 bg-violet-300/[.035] p-5">
      <p className="text-xs font-semibold uppercase tracking-wider text-violet-300">Mochila</p>
