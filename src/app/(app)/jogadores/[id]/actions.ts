...
export async function grantItemToPlayer(
  ...
  try {
    ...
    if (eggType) {
      ...
      // generatedByPlayerId = admin so the target player is not self-banned
      const genById = adminPlayer?.id ?? playerId;
      for (let q = 0; q < safeQty; q++) {
        await prisma.syncTicketHalf.create({
          data: { side, ownerId: playerId, generatedByPlayerId: genById, sourceAction: "admin-grant", status: "AVAILABLE" },
        });
      }
    } else if (item.type === SYNC_TICKET_TYPES.complete) {
      // SyncTicket requires two halves and two banned-user references.
      // We create synthetic halves owned by the player, generated by the admin
      // so the ban only affects the admin (not the target player).
      const genById = adminPlayer?.id ?? playerId;
      for (let q = 0; q < safeQty; q++) {
        const [leftHalf, rightHalf] = await Promise.all([
          prisma.syncTicketHalf.create({
            data: { side: SyncTicketSide.LEFT, ownerId: playerId, generatedByPlayerId: genById, sourceAction: "admin-grant", status: "COMBINED" },
          }),
          prisma.syncTicketHalf.create({
            data: { side: SyncTicketSide.RIGHT, ownerId: playerId, generatedByPlayerId: genById, sourceAction: "admin-grant", status: "COMBINED" },
          }),
        ]);
        await prisma.syncTicket.create({
          data: {
            ...
            bannedUserAId: genById,
            bannedUserBId: genById,
            status: "AVAILABLE",
          },
        });
      }
