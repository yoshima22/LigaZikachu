...
import type { GachaCurrency, Prisma } from "@prisma/client";
...
const missionSchema = z.object({
  ...
  source: z.enum([
    "ARENA_Z", "LIGA_RUSH", "LIGA_SEMANAL", "BATALHA_TERRENO", "ARENA_DRAFT",
...
export async function saveGachaMissionAction(input: z.infer<typeof missionSchema>) {
  ...
  const { id, ...data } = parsed.data;
  const values = { ...data, bannerId: data.bannerId || null, rarityFilter: data.rarityFilter || null };
