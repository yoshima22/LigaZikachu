...
export function LigaCashGiftForm({ senderPlayerId, products }: { senderPlayerId: string; products: Product[] }) {
  ...
  return (
    <section className="rounded-2xl border border-cyan-300/25 bg-gradient-to-br from-cyan-400/10 via-slate-950/80 to-violet-500/10 p-5 sm:p-6">
      <div className="flex items-start gap-3"><div className="rounded-xl border border-cyan-300/20 bg-cyan-300/10 p-2.5 text-cyan-200"><Gift size={20} /></div><div><h2 className="font-bold text-white">Comprar LigaCash de presente</h2><p className="mt-1 text-sm text-slate-400">Escolha um pacote e pague por PIX. Seu saldo de LC não será usado. Após a confirmação, o jogador verá sua mensagem na Caixa de presentes e poderá resgatar o pacote.</p></div></div>
