...
export function LigaCashGiftForm({ senderPlayerId, products }: { senderPlayerId: string; products: Product[] }) {
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="flex w-full items-center justify-between gap-3 rounded-2xl border border-cyan-300/25 bg-gradient-to-r from-cyan-400/10 via-slate-950/80 to-violet-500/10 p-5 text-left transition hover:border-cyan-300/50 hover:bg-cyan-300/10 sm:p-6">
        <span className="flex items-center gap-3"><span className="rounded-xl border border-cyan-300/20 bg-cyan-300/10 p-2.5 text-cyan-200"><Gift size={20} /></span><span><span className="block font-bold text-white">Comprar LigaCash de presente</span><span className="mt-1 block text-xs text-slate-400">Escolha um pacote, escreva uma mensagem e pague por PIX.</span></span></span>
      ...
      <div className="flex items-start gap-3"><div className="rounded-xl border border-cyan-300/20 bg-cyan-300/10 p-2.5 text-cyan-200"><Gift size={20} /></div><div><h2 id="liga-cash-gift-title" className="font-bold text-white">Comprar LigaCash de presente</h2><p className="mt-1 text-sm text-slate-400">Escolha um pacote e pague por PIX. Seu saldo de LC não será usado. Após a confirmação, o jogador verá sua mensagem na Caixa de presentes e poderá resgatar o pacote.</p></div></div>
