...
export default async function LigaCoinsPage(){
 ...
 return <div className="space-y-6">
  ...
  {player&&<LigaCashGiftForm senderPlayerId={player.id} products={products.filter(p=>!("adminOnly" in p&&p.adminOnly)).map(p=>({code:p.code,label:p.label,amount:p.base+p.bonus,cents:p.cents}))}/>}
