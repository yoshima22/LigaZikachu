...
import { adminReviveTowerMascotAction, getTowerLobbyDataAction, createTowerRunAction, joinTowerRoomAction, contributeTowerPreparationAction, spendTowerTalentAction } from "../actions";
...
export function TowerLobby() {
  ...
  return (
    <div className="space-y-6">
      ...
      {data.ranking.length>0&&<section className={card}><p className="text-[10px] font-black uppercase tracking-widest text-[#FFCB05]">Ranking da Torre</p><h2 className="mt-1 text-lg font-black text-white">Exploradores e resgatistas</h2><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[580px] text-left text-xs"><thead className="text-slate-500"><tr><th className="p-2">#</th><th>Jogador</th><th>Entradas</th><th>Resgates</th><th>Pontos conquistados</th></tr></thead><tbody>{data.ranking.map((row,index)=><tr key={row.userId} className="border-t border-slate-800"><td className="p-2 font-black text-[#FFCB05]">{index+1}</td><td className="font-bold text-white">{row.name}</td><td>{row.entries}</td><td>{row.rescues}</td><td>{row.talentPoints}</td></tr>)}</tbody></table></div></section>}
      ...
      <section className={card}>
        ...
        <div className="my-4 flex items-center gap-4 rounded-2xl border border-purple-300/25 bg-purple-950/15 p-3">{/* eslint-disable-next-line @next/next/no-img-element */}<img src="/events/torre-dos-rebeldes/chandelure.png" alt="Chandelure" className="h-24 w-24 shrink-0 object-contain"/><div className="rounded-2xl border border-purple-200/20 bg-black/35 p-3 text-xs leading-relaxed text-purple-50"><b className="block text-purple-300">Chandelure, o Arquivista</b>Isto não é debug. São estudos reais entre runs: cada contribuição diária soma ao esforço da Liga e, ao chegar a 5, libera uma contramedida para todas as expedições futuras.</div></div>
