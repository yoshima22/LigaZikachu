...
export function TowerLobby() {
  ...
  const [claimedMascot,setClaimedMascot]=useState<{name:string;sprite:string;level:number;personality:string;origin:string;stats:{statForce:number;statAgility:number;statCharisma:number;statInstinct:number;statVitality:number}}|null>(null);
  ...
  // Cooldown de entrada.
  if (data.nextEntryAt && data.pendingMascotRewards.length === 0 && !claimedMascot) {
  ...
  return (
    <div className="space-y-6">
      ...
      {claimedMascot&&<div className="fixed inset-0 z-[110] flex items-center justify-center overflow-y-auto bg-black/90 p-4"><section className="my-auto w-full max-w-2xl overflow-hidden rounded-[2rem] border border-fuchsia-300/50 bg-slate-950 shadow-[0_0_90px_rgba(217,70,239,.35)]"><div className="relative overflow-hidden bg-[radial-gradient(circle_at_center,rgba(217,70,239,.35),transparent_65%)] px-6 pb-6 pt-8 text-center"><div className="absolute inset-0 animate-pulse bg-[radial-gradient(circle_at_center,rgba(255,255,255,.12),transparent_45%)]"/><p className="relative text-[10px] font-black uppercase tracking-[.3em] text-fuchsia-200">✦ Um novo vínculo foi formado ✦</p><img src={claimedMascot.sprite} alt={claimedMascot.name} className="relative mx-auto mt-3 h-64 w-64 object-contain drop-shadow-[0_0_32px_rgba(232,121,249,.75)]"/><h2 className="relative text-3xl font-black text-white">{claimedMascot.name}</h2><p className="relative mt-1 text-sm font-bold text-fuchsia-200">entrou para sua equipe!</p></div><div className="border-t border-fuchsia-300/15 p-5"><div className="grid grid-cols-5 gap-2 text-center">{[["FOR",claimedMascot.stats.statForce],["AGI",claimedMascot.stats.statAgility],["CAR",claimedMascot.stats.statCharisma],["INS",claimedMascot.stats.statInstinct],["VIT",claimedMascot.stats.statVitality]].map(([label,value])=><div key={label} className="rounded-xl border border-slate-800 bg-slate-900/80 p-2"><small className="block text-[9px] text-slate-500">{label}</small><b className="text-base text-white">{value}</b></div>)}</div><div className="mt-4 grid gap-2 rounded-xl border border-fuchsia-300/20 bg-fuchsia-950/15 p-3 text-xs text-slate-300 sm:grid-cols-3"><span><b className="block text-fuchsia-200">Nível</b>{claimedMascot.level}</span><span><b className="block text-fuchsia-200">Personalidade</b>{PERSONALITIES.find(([value])=>value===claimedMascot.personality)?.[1]??claimedMascot.personality}</span><span><b className="block text-fuchsia-200">Origem</b>{claimedMascot.origin}</span></div><button onClick={()=>setClaimedMascot(null)} className="mt-5 w-full rounded-xl bg-gradient-to-r from-fuchsia-400 via-purple-400 to-cyan-300 py-3 text-sm font-black text-slate-950 shadow-[0_0_25px_rgba(217,70,239,.3)]">Conhecer meu novo companheiro</button></div></section></div>}
      ...
      {data.pendingMascotRewards.length>0&&<section className="overflow-hidden rounded-3xl border border-fuchsia-400/40 bg-gradient-to-br from-fuchsia-950/35 via-slate-950 to-purple-950/30 shadow-[0_0_55px_rgba(217,70,239,.15)]"><div className="border-b border-fuchsia-300/15 p-6"><p className="text-[10px] font-black uppercase tracking-[.24em] text-fuchsia-300">Um rebelde decidiu acompanhar você</p><h2 className="mt-1 text-2xl font-black text-white">Escolha a personalidade antes do nascimento</h2><p className="mt-2 text-xs leading-relaxed text-slate-300">A origem será de Laboratório. O crescimento é simulado nível a nível até o Nv.55, portanto a personalidade escolhida participa de toda a formação dos atributos.</p></div><div className="grid gap-4 p-5 md:grid-cols-2">{data.pendingMascotRewards.map(reward=><article key={reward.id} className="relative overflow-hidden rounded-2xl border border-fuchsia-300/25 bg-black/30 p-4 text-center"><div className="absolute inset-x-0 top-0 h-24 bg-fuchsia-500/10 blur-2xl"/><img src={reward.sprite} alt={reward.name} className="relative mx-auto h-36 w-36 object-contain drop-shadow-[0_0_18px_rgba(232,121,249,.45)]"/><b className="relative block text-lg text-fuchsia-100">{reward.name}</b><select value={rewardPersonalities[reward.id]??"LOYAL"} onChange={event=>setRewardPersonalities(current=>({...current,[reward.id]:event.target.value}))} className="relative mt-3 w-full rounded-xl border border-fuchsia-300/25 bg-slate-950 p-2.5 text-xs text-white">{PERSONALITIES.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><button disabled={pending} onClick={()=>start(async()=>{const result=await claimTowerMascotRewardAction(reward.id,(rewardPersonalities[reward.id]??"LOYAL") as never);if("error"in result)toast.error(result.error);else{setClaimedMascot(result.mascot);load()}})} className="relative mt-3 w-full rounded-xl bg-gradient-to-r from-fuchsia-400 to-purple-400 py-2.5 text-xs font-black text-slate-950 shadow-[0_0_22px_rgba(232,121,249,.25)] disabled:opacity-40">✨ Receber mascote</button></article>)}</div></section>}
