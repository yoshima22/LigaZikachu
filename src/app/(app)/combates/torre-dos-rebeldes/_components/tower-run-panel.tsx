    ...
    <section className={card}>
      ...
      {run.status === "ACTIVE" && (
        <div className="mt-4 space-y-4">
          ...
          {state.exploration?.lastOutcome && <div className="flex items-center gap-3 rounded-xl border border-purple-400/25 bg-purple-950/20 p-3">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={state.scene?.characterUrl || "/events/torre-dos-rebeldes/chandelure.png"} alt={state.scene?.speaker || "Chandelure"} className="h-16 w-16 shrink-0 object-contain"/><p className="text-sm text-purple-100"><b className="mb-1 block text-[10px] uppercase tracking-widest text-purple-300">{state.scene?.speaker || "Chandelure, o Arquivista"}</b>{state.exploration.lastOutcome}</p></div>}
