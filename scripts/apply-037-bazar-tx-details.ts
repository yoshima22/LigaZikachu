/** Aplica a migração 037 (detailsJson em bazar_transactions). Idempotente. */
...
(async()=>{
  const stmts=readFileSync("prisma/migrations-manual/037_bazar_transaction_details.sql","utf8").split(/\r?\n/).map(s=>s.trim()).filter(s=>s.startsWith("ALTER")).map(s=>s.replace(/;$/,""));
  for(const s of stmts) await prisma.$executeRawUnsafe(s);
  console.log("Migração 037 aplicada:",stmts.length,"statements.");
