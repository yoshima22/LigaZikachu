/** Aplica a migração 036 (disableProfileIntro + eggRarities na wishlist). Idempotente. */
...
(async()=>{
  const stmts=readFileSync("prisma/migrations-manual/036_profile_intro_wishlist_rarities.sql","utf8").split(/\r?\n/).map(s=>s.trim()).filter(s=>s.startsWith("ALTER")).map(s=>s.replace(/;$/,""));
  for(const s of stmts) await prisma.$executeRawUnsafe(s);
  console.log("Migração 036 aplicada:",stmts.length,"statements.");
